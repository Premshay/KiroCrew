...
| Agent pytest wrapper | `scripts/run_agent_pytest.py`, `xdist_budget.py`, `sandbox.py` (`pytest_cgroup_scope_argv`) | Every backend test command launched by the shipped local-gate/scoped-test paths | Six host-global advisory-lock permits. The launcher validates the `-n` count and starts a waited transient systemd service; the service re-enters the script as a shim that atomically reserves exactly that many permits, keeps those fds open, and is `pytest`'s parent, passing `PREGRANTED_WORKERS` to prevent a second auto-mode claim. The holder is inside the unit because a transient service outlives its starter: a launcher-held lease would be released the moment the launcher died, while its workers kept running. The service cgroup applies `memory.high=16 GiB`, `memory.max=20 GiB`, `memory.swap.max=0`, and `OOMPolicy=kill` (systemd's `memory.oom.group=1` mapping). Explicit requests above six are refused; unavailable permits fail with a retryable busy status, never silently downsize. | Yes on delegated cgroup-v2 hosts. The cgroup throttles first and OOM-kills the entire test tree at its hard ceiling; the lock pool makes six total workers real across worktrees. | A test spike cannot consume the last host memory with the gateway/model server. If cgroup delegation is absent, the wrapper still enforces the six shared permits and emits the existing SECURITY warning; cgroup memory containment is unavailable. |
