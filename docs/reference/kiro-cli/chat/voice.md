...
Set **Language** in the Speech-to-Text card before recording (`stt.language_code`
in config). The recognizer is given that explicit language instead of guessing
one, so Hebrew is recognized as Hebrew rather than as English.
...
| `system` | nothing on macOS and Windows | The host's own engine: `say` on macOS, `System.Speech` through Windows PowerShell on Windows, `espeak-ng` (or legacy `espeak`) on Linux/BSD when installed. Linux is the one platform where it can be missing; install `espeak-ng` or pick another provider. |
