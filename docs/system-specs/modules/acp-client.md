...
- an adapter on the Claude control-plane seam skips `session/set_mode`; its declared
  `model_switch_method` selects either `session/set_config_option` (configId `model`)
  or `session/set_model`. Claude uses the config-option route.
...
opted-in direct construction sites. It merges `extra_env` while retaining the
caller's sandbox, work directory, MCP overlay, and audit policy. An unbound
runtime retains the native Kiro CLI path. A claude binding resolves to
`acp/harness/claude.py` (`ClaudeHarness`), registered for these direct
consumers only — `ACP_BACKENDS_ACP_RUNTIME` still keeps provider claude sessions
on `AcpClient`. The adapter takes no model flag, so its `SpawnPlan` carries
`session_model` (applied after `session/new` through `session/set_model` when
`model_switch_method == "session_set_model"`, else `session/set_config_option`)
and `session_meta` (the `claudeCode` envelope); it seeds no
`settings.local.json` and builds no mirror projection, so a pooled MCP array
is refused on this path.

**Session MCP reports.** Both ACP transports retain the session-local MCP
initialization and OAuth notifications in `McpSessionReport`; they are exposed
through the session handle/provider rather than inferred from the configured
agent spec. A bounded startup drain and later in-turn events update the same
report, so a missing report is unknown rather than evidence that a server was
not mounted.
