...
Stopping an Autopilot slot cancels its outer stage controller as well as settling
the captured turn task after a hard or idle provider stop. Peer-delivery callbacks
remain attached when a stage-owned queue entry is consumed. Replacing a plan
clears the stage boundary and records changed plan metadata in the session timeline.
Peer interrupts steer or cancel only an executing turn. A paused stage keeps its
reservation and queues the peer message without calling the idle provider.
