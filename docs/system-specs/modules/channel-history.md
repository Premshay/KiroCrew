...
The budget governs provider-owned agents only. Attached persistent dashboard
peers are exempt: their reports must receive either an explicit delivery receipt
or an explicit failure outcome. When the provider-agent budget defers a
delivery, the original channel message persists a `backpressure` receipt for
that recipient and the sender receives a non-waking system notice, which keeps
the loop guard while making the deferred work visible to the party that
attempted it.
