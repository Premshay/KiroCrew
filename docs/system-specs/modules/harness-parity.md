...
Private engine bindings retain Claude and DeepSeek steer support through
`ACP_BACKENDS_STEER`. Codex uses the shared runtime and its harness-owned MCP
projection; Claude and DeepSeek harnesses remain available to direct runtime
consumers without changing their ordinary provider routing. Engine-bound model
switching and session-map label overrides coexist with the upstream identity API.
