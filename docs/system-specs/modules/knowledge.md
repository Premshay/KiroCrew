...
- **Knowledge embedding dispatch is bounded with scalar vector arithmetic** — `InProcessEmbedder.embed_for_items` shares the singular item's text assembly and calls the backend once per item. File/text ingestion and the sig-gated rebuild path dispatch groups of 32 to amortize executor hops while retaining per-item SQLite writes, compare-and-set protection, retry stamps, job heartbeats, and bit-identical query/document vectors.
