...
**Terminal batch failures.** `send_batch()` trusts the ACP exception's existing
`transient` verdict: when it is exactly `False`, it stops dispatching items still
waiting for a pool slot, returns empty results for them, and logs the terminal
cause once. It does not create a second classifier or suppress a later sweep.
Items already handed to workers may still finish. A `True` or absent verdict
retains per-item failure handling, so temporary and unclassified failures do not
cancel their peers.
...
- **Knowledge embedding dispatch is bounded, not serial** — `InProcessEmbedder.embed_for_items` shares the singular item's text assembly and content budget, then delegates a bounded group to `EmbeddingBackend.embed_batch`. File/text ingestion and the sig-gated rebuild path dispatch groups of 32 while retaining per-item SQLite writes, compare-and-set protection, retry stamps, and job heartbeats. The group limit bounds gateway work only; a backend remains responsible for its own token/context splitting.
