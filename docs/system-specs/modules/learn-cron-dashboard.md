...
Dashboard command discovery and dispatch route the Claude command registry through
the SDK session capability record
(`capabilities_of(provider).provider_seam`), preserving the curated command list
for other provider seams. Turn usage records also retain the slot's app identity,
independently of the resolved agent and linked-session surface.
...
**Fork integration contracts:** Peer delivery references and consumption callbacks
remain attached to queue dispatch while the session ledger derives its actor from
the same enqueue-time kind. The append-only ledger records turn facts; it does not
replace the slot checkpoint, peer inbox, or post-restart continuation used by
Multiplex. Context composition supplies both the live provider and the retained
post-compaction checkpoint. Provider stream creation follows authorization,
shutdown, and Stop gates; replay defers native-ID publication until a landed turn.

**Direct-turn durability:** `POST /api/chat` persists the user row before it
starts provider work or acknowledges acceptance, and `_run_chat` flushes the
completed reply before settling the turn. If admission persistence fails, the
request is rejected without starting provider work, so an accepted direct prompt
or a completed reply is not lost to a watchdog exit.
