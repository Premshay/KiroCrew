...
**Session-directive consumption** — an optional `directive_consumer` callback (`(kind, args) -> awaitable`) makes the driver the channel-side consumer of the stateless session-directive protocol (`session_directive.py`). At `EVENT_TOOL_CALL`, the shared forgery gate records an identity only when adapter-owned provenance confirms the `kirocrew-core` server/tool pair: Kiro uses `_meta.kiro`, Claude uses its adapter-owned identity, and Codex requires its structured server/tool identity plus MCP-call provenance. The model-authored title is never an identity source. The matching `EVENT_TOOL_RESULT` marker is then decoded and delivered once; forged markers under another tool are ignored, and a native-subagent tool call is denied and SEL-audited so a child cannot mutate its parent. Dispatchers inject `messaging.dispatch.build_directive_consumer(session_key=…, sessions=…, dispatcher=…)`, which calls the same `apply_session_directive` core as the dashboard with `slot=None`; dashboard-card directives therefore remain unavailable to channel turns. Channel `set_project` persists the per-conversation project/CWD override and the next claimant cold-starts in that directory. Legacy monitor starts may use Slack, Discord, or Webex; structured monitors require typed dispatch plus completion correlation and are supported only for dashboard, Slack, and Discord. Other transports are refused before arming and logged/audited rather than silently dropped. Without a consumer, directive markers remain inert.
