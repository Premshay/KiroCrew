...
`src/kiro_crew/channel.py`, `src/kiro_crew/dashboard/handlers_channel.py`, and `website/src/pages/ChannelPage.tsx` implement persistent multi-agent workspaces where an orchestrator coordinates specialist agents through channel messages, threads, and @mentions. Attached dashboard sessions are the
...
- Existing dashboard sessions attached as collaboration peers, with typed peer
  reports and delivery receipts: `progress`, `mention`, and `done`.
...
ChannelAgent
  ...
  ├── attached_session: whether the member is an attached dashboard session
...
6. An attached-session named request produces one queued synthetic peer turn, or
   an explicit interrupt steer with a durable queue-head fallback; passive
   `progress` and `done` reports wait for the recipient's next normal turn.
   Attached persistent peers are exempt from the directed exchange budget in
   rule 5, which governs provider-owned agents only.
...
`api_channel_create` validates supplied agent fields and injects an orchestrator when none is marked. That injection applies only to a preset that includes agents; the EMPTY preset creates a session-only channel instead, containing no provider-owned agent and opening the live-session picker immediately, so an operator can attach existing dashboard sessions without assigning replacement roles. `ChannelPage.tsx` sends the agents for the selected preset ID rather than resolving roles from display text; its local preset list is only a fallback when the endpoint fails.
...
The empty preset opens the session picker immediately: it lists live dashboard
slots and omits channel members already attached. Dismiss remains available for
terminal members — a successful dismissal removes the member from the visible
list and frees channel capacity — while clear context remains limited to live
members.
...
| POST | `/api/channels/{id}/sessions` | Attach one existing dashboard session as a channel peer. |
