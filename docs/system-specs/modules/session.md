...
- **Consumers**: STRICT identity resolvers accept the direct
  ...
  bind any effect to the key it returns. `session_checkpoint` uses the same
  directive transport without resolving a session identity. Lenient (read-only)
...
The applier reuses the SAME effect cores the HTTP endpoints call — `authorize_and_add_nudge` / `authorize_and_update_nudge` / `svc.remove` for the monitor trio, `slot.project` plus the recent-projects save for `set_project`, `deliver_ws_owners` for `suggest_followup`, `post_question_card` for `ask_question`, and the slot's persisted checkpoint projection for `session_checkpoint` — so behavior is unchanged except that `ask_question` is now non-blocking (full contract in `learn-cron-dashboard.md` → "Agent Questions"). `session_checkpoint` is intentionally a low-frequency operator aid: it replaces one short current-work summary, one explicit next action, and up to four main items, appends one milestone to a seven-entry trail, and may include one validated plan or goal count. It may additionally raise an explicit unassigned attention request, optionally keyed as a decision; the agent cannot claim or resolve that request. Whenever work remains, the author includes a concrete next action; it may be omitted only when no known next step exists. The next action clears when a later checkpoint omits it, while omitted attention remains until the author explicitly clears it, so Multiplex cannot silently lose an operator decision. It does not summarize the transcript, call an LLM, or carry a session key.
