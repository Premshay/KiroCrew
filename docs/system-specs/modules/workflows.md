...
Authoring retains component-size diagnostics for the rendered intent and saved
references before applying the private-memory prompt wrapper. Turn identity is
published and the provider context contract is applied before generation; prompt
size diagnostics do not replace either gate.
