...
def backend_argv(targets: list[str] | None) -> list[str]:
    ...
    """
    argv = [sys.executable, str(REPO_ROOT / "scripts" / "run_agent_pytest.py"), "-q", *pytest_parallel_args()]
    if targets:
...
def _self_test() -> int:
    ...
    check(
        "backend full argv",
        backend_argv(None) == [sys.executable, str(REPO_ROOT / "scripts" / "run_agent_pytest.py"), "-q", *pytest_parallel_args()],
    )
    try:
