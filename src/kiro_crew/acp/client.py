...
def acp_config_option_values(config_options: object, *option_ids: str) -> list[str]:
    ...
    return [
        ...
        if isinstance(choice, dict) and isinstance(choice.get("value"), str) and choice["value"]
    ]
...
class AcpClient:
    ...
    def _member_mount_withheld(
        ...
        *capability* is the phrase the log uses for what the session loses, because
        ...
        if not acp_tool_gate.is_enforced(self.backend) and not _permission_surface_owned(self):
            logger.warning(
    ...
    def _goose_session_mcp_servers(self) -> list:
        ...
        **The drop is unobservable on this harness, and the corpus pin is the accepted
        ...
        """
        servers = self._translated_session_mcp_servers()
        logger.info(
