...
class TestAdoption(_Base):
    ...
    def test_adoption_requires_the_dispatch_capability_when_requested(self):
        ...
        self.assertTrue(
            results.dispatch_result_path("CR-1", "correct-capability", self.root).exists()
        )
        self.assertIsNone(results.read_result("CR-1", self.root, "run-a"))
    ...
    def test_non_ascii_capability_is_refused_without_aborting_adoption(self):
        record = _record()
        record["result_capability"] = "malformed-\u2603"
        path = results.dispatch_result_path("CR-1", "expected-capability", self.root)
        path.parent.mkdir(parents=True)
        path.write_text(json.dumps(record), encoding="utf-8")

        self.assertFalse(results.adopt_from_shared(
            "CR-1", self.root, "run-a", expected_capability="expected-capability"
        ))
        self.assertIsNone(results.read_result("CR-1", self.root, "run-a"))
...
class TestStakedSlot(_Base):
    ...
    def test_an_unclearable_slot_is_not_adopted_from(self):
        ...
        real_stake = results.stake_dispatch
        results.stake_dispatch = lambda change_id, capability, root=None: False  # EPERM
        try:
            ...
            results.stake_dispatch = real_stake
