...
```bash
python scripts/run_agent_pytest.py -q  # host-wide xdist budget + setup.cfg addopts
...
- To omit coverage during iteration without dropping the parallel safeguards:
  ...
  ```bash
  python scripts/run_agent_pytest.py -q --override-ini="addopts=--ignore=build/private -n auto --dist loadgroup --max-worker-restart=2"
  ```
