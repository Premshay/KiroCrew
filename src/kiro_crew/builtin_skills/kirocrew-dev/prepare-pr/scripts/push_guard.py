...
# characters (git/ssh stderr wraps remotes in quotes/parens).  The host
# portion must look like a hostname (contains a dot OR a colon-path to
# distinguish from local usernames in non-URL contexts like "user@localhost").
#
...
_SCP_USERINFO_RE = re.compile(
    ...
    r"@"
    r"([^\s/:@\"'`)\]]+\.[^\s/:@\"'`)\]]+)"  # group 2: host (must contain a dot — distinguishes from bare user@word)
)
...
def redact_credentials(text: str) -> str:
    ...
    3. Scheme-less SCP-style userinfo: replaces ``user@host`` in tokens that
       lack a ``://`` scheme (e.g. ``tok@git.example.com:team/Repo.git`` as
       echoed by git-fetch/ssh failure stderr) with ``<redacted>@host``.
    ...
    - SCP-style: ``<redacted>@host:path`` (or ``<redacted>@host``).
...
def _fetch_base(base):
    ...
    if fetch_rc != 0:
        err(
            ...
            # Redact before truncate: a slice boundary must never split a
            # credential out of regex reach.
