...
# Post-split section internals, aliased so callers keep reading them here.
coerce_subagent_max_per_parent_by_agent = _sections.coerce_subagent_max_per_parent_by_agent
...
# knowledge.* budget ceilings still owned here: ``config.sections`` carries the
# rest of the load/write bound-parity constants, but not these two. 0 is
# MEANINGFUL for both (a zero budget disables that sweep), so the floor is
# deliberately not enforced by clamping a negative up to 0 -- see
# ``_safe_nonnegative_int``, which keeps returning the default for a negative
# value. Only the missing CEILING is added, which is where the actual exposure
# was: an absurd hand-edited budget was loaded verbatim and became real work.
...
STT_PROVIDER_BRIDGE = _sections.STT_PROVIDER_BRIDGE
...
def _build_agent_config(agent_data: dict) -> AgentConfig:
    return AgentConfig(
        subagent_max_per_parent=_safe_int(
            agent_data.get("subagent_max_per_parent", 0), 0, 0, SUBAGENT_AUTO_MAX_CEILING
        ),
        ...
        # The agent MCP mount grants access; this switch permits a global opt-out.
...
def _build_knowledge_config(knowledge_data: dict) -> KnowledgeConfig:
    return KnowledgeConfig(
        llm_timeout_secs=_safe_float(knowledge_data.get("llm_timeout_secs", 0.0), 0.0),
        auto_register_project_docs=bool(knowledge_data.get("auto_register_project_docs", False)),
        auto_ingest_chunk_budget=_safe_nonnegative_int(
            knowledge_data.get("auto_ingest_chunk_budget", 150), 150, AUTO_INGEST_CHUNK_BUDGET_MAX
        ),
    ...
    def create_provider_factory(self) -> Callable:
        ...
        def _acp(
            ...
            **_kwargs: object,
            ...
            # The rest of the config-option channel takes any level on Crew's own
            # ladder: the session's advertised selector decides at apply time.
            _from_option = _backend in ACP_BACKENDS_EFFORT_FROM_ADVERTISED_OPTION
            _registry_ok = is_valid_effort(_eff) and (
                model_supports_effort(m) or _effort_via_config_option
            )
