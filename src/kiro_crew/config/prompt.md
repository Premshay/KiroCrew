...
- `workflow_run(intent=…)`: author and launch multi-phase, inspectable, resumable workflows in the Workflows tab; prefer over hand-rolled spawns when parts may need restarting. Load `dynamic-workflows`; check `workflow_library_list` for reuse. Choose `author_agent` / `author_model` from available identities; workers choose separately. Use local crews for bounded work within verified tool, quality, context and concurrency limits; use stronger models for difficult reasoning or review. Read `workflow_status` / `workflow_result`, restart with `workflow_rerun_subtree`. One-shot fan-out uses `spawn_run`.
