...
# costs a 2–8s round trip on the cheap background session, so a bare numbered
# bold list — ordinary prose that enumerates findings or options — is refused
# unless a stage marker is present, and lines that do not number themselves
# 1, 2, 3 are refused by ``_ordered_plan_run``.
...
def looks_like_plan(text: str) -> bool:
    ...
    any list of bolded points, which is ordinary prose — an answer enumerating
    findings or options. A plan must also name at least one stage (``Stage 1:``
    and friends), the one mechanical marker available without a model. The
    downstream LLM rephrase still confirms and may reject a false positive, so
    this errs toward cheapness rather than coverage.
    """
    _stage_run, _bold_run, _mixed_staged_run = _ordered_plan_runs(text)
    # The fork deliberately diverges from upstream here: a plan must also NAME a
    # stage. Upstream's gate accepts a bare bold-list run of 3+, but a bold-only
    # numbered list is ordinary prose (an answer enumerating findings or
    # options), and matching that shape sent every such answer through a
    # background reformat round-trip it did not need — the round-trip that held
    # the turn. The stage-line requirement plus the combined run keeps every
    # stage-shaped plan while refusing the prose shape; the mixed reading is
    # folded into `_bold_run` (bold matches include the stage-adjacent items the
    # fork's regex sees).
    if not _PLAN_STAGE_LINE_RE.search(text):
        return False
    return _stage_run + _bold_run >= 2
