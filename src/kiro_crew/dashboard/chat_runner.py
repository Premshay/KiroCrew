...
from kiro_crew.trust_patterns import approval_command, approval_display_command
    ...
    try:
        ...
        if is_slash:
            ...
            if _context_is_new and not _provider_has_history:
                ...
                # must not burn the one chance the reset asked for. Only the
                # concrete boolean from SessionManager authorizes an empty replay:
                # treat an incomplete adapter or test double as no suppression,
                # since losing existing conversation context is the unsafe failure.
        ...
        if _retry_eligible:
            ...
            session_ledger_emit.on_model_selected(
                _ledger_sid, _fallback_model, "fallback", turn=_ledger_turn_no
            )
            ...
            # moved the live session onto `_fallback_model` via the substitute
