...
# Trust-pattern matching, base/full command extraction and segment splitting
# now live in :mod:`kiro_crew.trust_patterns` (imported above under their old
# private names). This module's inline copies were dropped rather than kept:
# upstream's ``extract_base_command`` additionally refuses env-assignment
# prefixes and quote/expansion characters in a base, returning no grant where
# ours fell back to the first token.
...
async def _deliver_cross_surface_reply(state: Any, session_key: str, assistant_text: str) -> None:
    ...
    # Apply context-aware and display-form redaction before the reply crosses
    # into a renderer-less transport.
    ...
    try:
        ...
        if is_slash:
            ...
            if (
                ...
                # Spend the one-shot only at the real cold-start replay gate:
                # a warm turn must not consume a deliberately fresh restart.
                # Only the concrete boolean from SessionManager authorizes an
                # empty replay.  Treat an incomplete adapter or test double as
                # no suppression: losing existing conversation context is the
                # unsafe failure mode.
                if state.sessions.consume_replay_suppression(session_key) is True:
                    logger.info("Session replay suppressed by explicit reset: %s", session_key)
                else:
        ...
        async for event in event_stream:
            ...
            if event.kind == EVENT_TEXT_CHUNK:
                ...
                if (
                    ...
                    continue
                # Session-trusted patterns are matched only against the canonical
                # execution identity, never model-authored display text.
                if (
