...
_DASHBOARD_COMMANDS = ("/side", "/goal", "/prompts", "/workflow")
...
def _advertised_commands(state: DashboardState | None, slot: str | None) -> list[dict[str, str]]:
    ...
    for provider in providers:
        if not is_claude_code(capabilities_of(provider).provider_seam):
            continue
        commands = getattr(provider, "slash_commands", [])
        if not commands:
            continue
        return [
            {"name": f"/{c['name']}", "description": _menu_description(c["description"])}
            for c in commands
            if f"/{c['name']}" not in _BLOCKED_SLASH_COMMANDS
        ]
    return []
...
async def api_slash_commands(request: web.Request) -> web.Response:
    ...
    if not result and is_claude_code(cfg.agent.provider):
...
async def api_kirocrew_agents(request: web.Request) -> web.Response:
    ...
    for name, agent_cfg in cfg.agents.items():
        row = _agent_roster_row(name, "global", agent_cfg, redact=redact)
        # Companion-owned UI policy, carried per global row. It is derived from
        # the platform context rather than the stored record, so it is attached
        # here rather than inside the row allowlist above.
