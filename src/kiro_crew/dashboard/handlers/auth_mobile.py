...
def _caller_bounds(request: web.Request) -> tuple[dict[str, str], int]:
    """Read the caller's own session bounds from its access cookie.

    Returns ``(carried_claims, ttl_ceiling_seconds)``. The middleware has
    already authenticated the request, so the cookie's signature is verified;
    this only re-reads the payload the middleware does not expose. Claims are
    carried, never re-derived: ``boot`` copied verbatim (same rule as the
    link→session exchange in ``token_auth``), ``no_refresh`` copied so the
    recipient session never grows a refresh chain, and the remaining
    ``session_exp`` becomes the TTL ceiling so a short-lived caller cannot
    mint a longer-lived credential. Fail-closed on an unreadable payload:
    a caller whose bounds cannot be established gets a bounded (no-refresh,
    default-TTL-capped) link rather than an unbounded one.
    """
    port = request.app.get("port", 7777)
    cookie_name = f"mc_token_{_cookie_port_from_host(request, port)}"
    token = request.cookies.get(cookie_name, "") or request.query.get("token", "")
