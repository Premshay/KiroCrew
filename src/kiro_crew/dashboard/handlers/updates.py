...
# MERGE-REVIEW: upstream imports UNREADABLE_TIMEOUT / DivergenceUnreadable /
# count_divergence from kiro_crew.git_divergence and calls them at the two
# ahead/behind sites below. The fork inlined equivalent `git rev-list
# --count --left-right` logic at both (with its own ERR_GIT_READ_FAILED
# handling), so those names would be dead here and the import is omitted.
# Re-pointing both sites at the shared helper is a follow-up, not a merge fix.
