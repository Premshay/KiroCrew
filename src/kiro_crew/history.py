...
from kiro_crew.history_cache import (  # noqa: F401 - facade re-exports
    _METADATA_CACHE_MAX,
...
from kiro_crew.history_consolidation import (  # noqa: F401 - facade re-exports
    ...
    _CONSOLIDATION_CANCEL_ACK_TIMEOUT_S,
    _CONSOLIDATION_CHUNK_MAX_CHARS,
    _CONSOLIDATION_LANE_BUSY_RETRY_S,
    _CONSOLIDATION_MAX_ATTEMPTS,
    _CONSOLIDATION_META_KEYS,
    _CONSOLIDATION_NO_LANE_RETRY_S,
...
class ConversationLog:
    ...
    def __init__(
        ...
        *,
        ...
        self._msg_cache: _LRUCache[tuple[tuple[float, float, int], int, list[dict]]] = _LRUCache(
            cache_max
        )
        ...
        self._meta_cache: _LRUCache[tuple[tuple[float, float, int], int, dict]] = _LRUCache(
            _METADATA_CACHE_MAX
        )
        ...
        #: call. This memoizes the formatted window; the stored file identity
        #: and generation guard staleness together — an append bumps the
        #: identity, and a preserved-mtime rewrite through another instance is
        #: caught by the generation. Own ``_LRUCache`` → own internal lock.
        self._recent_cache: _LRUCache[tuple[tuple[float, float, int], int, list[dict]]] = (
            _LRUCache(cache_max)
        )
        ...
        self._folded_cache: _SearchTextCache[
            tuple[tuple[float, float, int], int, int, str]
        ] = _SearchTextCache(
        ...
        self._snippet_cache: _SearchTextCache[
            tuple[tuple[float, float, int], int, list[str]]
        ] = _SearchTextCache(
            _SEARCH_SNIPPET_BUDGET_BYTES,
