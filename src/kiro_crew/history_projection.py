...
from kiro_crew.history_cache import _cache_identity, _FileChangeCacheEntry
...
class TranscriptReadProjection:
    ...
    def _read_messages(self, key: str) -> list[dict]:
        ...
        if ident is not None:
            ...
            if cached and cached[0] == ident and cached[1] == self._log._cache_gen(key):
                return cached[2]
        ...
        *,
        ...
        for attempt in range(attempts):
            ...
            if gen is None or (
                ...
                self._log._msg_cache[key] = (ident, entry_generation, messages)
            return messages
        return []
    ...
    def _recent_via_tail(
        ...
        # The generation half is what a preserved-mtime rewrite through another
        # instance trips; this memo feeds recent(), the per-turn model-context
        # path, so a stale window here is served on every turn.
        if recent is not None and recent[0] == ident and recent[1] == self._log._cache_gen(key):
