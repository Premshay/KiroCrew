...
from kiro_crew.history_cache import _cache_identity
...
class SessionCatalogProjection:
    ...
    def _folded_content(self, key: str) -> tuple[int, str]:
        ...
        with self._log._file_lock(key):
            ...
            if cached and cached[0] == ident and cached[1] == self._log._cache_gen(key):
                return (cached[2], cached[3])
            built = self._log._build_folded(key, ident, gen)
            if built is None:
                ...
                return (0, "")
            self._log._publish_if_current(
                self._log._folded_cache, key, (ident, gen, built[0], built[1]), key=key, gen=gen
            )
            return built
    ...
    def _snippet_texts(self, key: str) -> Iterator[str]:
        ...
        if cached is not None:
            try:
                ident_now = _cache_identity(self._log._path(key).stat())
                if cached[0] == ident_now and cached[1] == self._log._cache_gen(key):
