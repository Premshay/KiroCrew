...
def _policy_session_key() -> str | None:
    ...
    * ``None`` — resolution itself broke (an unreadable home, a raising probe). That
    ...
    2. The optional host process identity. ``None`` means no identity was supplied;
       an EMPTY string refuses resolution instead of falling through to a token,
       environment variable or PID file. This hook grants no member-store authority.
