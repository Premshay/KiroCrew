...
class SessionClosingError(RuntimeError):
    ...
    once ``close_all`` has set ``_closing``) and by ``SessionManager.begin_turn``
...
class SpeculativeResumeRefused(RuntimeError):
    ...
    which preserves the observation by arming the session's one-shot
    ``FirstTurnState.RESUMED`` for the first real claimant. Without the opt-in
    the refusal stands: the real first turn needs to observe ``resumed=True``
    to make its history-injection decision, and existing-session reuse would
    report ``resumed=False``. Raised on the same session-map read that would
    drive the resume, so there is no window for a mapping to appear between a
...
class AllocationDeps:
    ...
    session_crew_effort: Callable[[Any, str | None], str | None]
    runtime_client_binding: Callable[[str], dict[str, Any]]
    apply_runtime_client_binding: Callable[[dict[str, Any], dict[str, Any]], None]
    ...
    async def get_subagent_runtime(self, parent_session_key: str, agent: str | None = None) -> Any:
        ...
        while True:
            ...
            async with lock:
                ...
                # Per-agent ACP client binding, applied after the parent's
                # security posture so the companion runtime lands on the same
                # binding the agent is configured for.
                self._deps.apply_runtime_client_binding(
                    kwargs, self._deps.runtime_client_binding(selected_agent)
                )
    ...
    def context_info(self) -> list[dict[str, object]]:
        ...
        for key, session in self._sessions.items():
            ...
            if self._deps.is_claude_provider(provider):
                ...
                # Any other provider answers through the provider-native
                # contract rather than a backend-private attribute, so a
                # non-Claude, non-ACP session reports its real model and agent
                # instead of a literal "unknown".
                model = getattr(provider, "served_model", "") or "auto"
                agent = getattr(provider, "agent_name", "") or ""
    ...
    async def get_or_create(
        ...
        **extra_factory_kwargs: Any,
        ...
        try:
            async with self._lock:
                ...
                if existing is not None and not recycling:
                    ...
                    if alive:
                        session.last_used = time.monotonic()
                        # Same provider-native store as the cold path. It is
                        # idempotent (returns False when the map already agrees),
        ...
        if "reasoning_effort_override" not in extra_factory_kwargs:
            crew_effort = self._deps.session_crew_effort(owner._cfg, agent)
            if crew_effort:
        ...
        is_stateless = (
            key
            in (
                constants.background_key,
                constants.heartbeat_key,
                constants.consolidate_key,
            )
