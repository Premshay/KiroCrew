...
class BackgroundSessionRuntime:
        ...
        for attempt in range(max_retries + 1):
            async with self._bg_runtime_lock:
                ...
                if runtime_capable and (
                    ...
                    # Sage review pool). Skipping it here would leave the
                    # background runtime on the default binding while every
                    # other runtime honoured the agent's.
                    self._deps.apply_runtime_client_binding(
                        runtime_kwargs, self._deps.runtime_client_binding(self._deps.runtime_agent)
                    )
                    runtime = AcpRuntime(agent=self._deps.runtime_agent, **runtime_kwargs)
                    await runtime.spawn()
    ...
    async def recycle_background(
        self, session_key: str | None = None, *, agent: str | None = None
    ) -> None:
        """Recycle a background-style provider when its context is full.

        Parameterised by key and agent because ``_bg`` is no longer the only
        stateless background session: ``_hb`` and ``_consolidate`` run the same
        no-compaction, swap-in-a-fresh-provider lifecycle under their own
        identities, and ``llm_helpers.background_turn`` recycles whichever key
        it borrowed. Both arguments default to the ``_bg`` pair, so the
        historical no-argument call is unchanged.
        """
        ...
        try:
            ...
            logger.info("Recycling background session %s — %s", background_key, reason)
