...
class CleanupState:
    ...
    #: Dashboard-supplied predicate answering "does this key still have
    #: non-turn work?" (a pending subagent, for instance). An idle session it
    #: vouches for is preserved: idleness is measured from ``last_used``, which
    #: only a turn bumps, so background work owned by the session is invisible
    #: to the clock and would otherwise be reaped mid-flight.
    idle_expiry_guard: Callable[[str], bool] | None = None
            ...
            for key, session in self._owner._sessions.items():
                ...
                if idle and guard is not None:
                    try:
                        ...
                        # Fail CLOSED: a guard that cannot answer must not be
                        # read as "no work here". Expiring on a guard fault is
                        # the unrecoverable direction — it kills a session that
                        # may be mid-flight — while preserving one costs a
                        # sweep.
                        self._deps.logger.warning(
