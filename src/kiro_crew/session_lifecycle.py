...
class _ChildTeardownHandler(Protocol):
    ...
    def running_agents_for(self, parent_session_key: str) -> Any: ...
        ...
        while True:
            async with owner._lock:
                    ...
                    if probe_awaited:
                        ...
                        if self._child_teardown is not None:
                            ...
                            if children is None or children:
                                return False
                        answer: bool | Awaitable[bool] = False
                    else:
                ...
                if not isinstance(answer, Awaitable):
                    ...
                    self._discard_replay_gap(key)
                    self.state.orphaned_holders.pop(key, None)
                    forget_generation = getattr(
                        self._child_teardown, "forget_attachment_generation", None
                    )
                    if callable(forget_generation):
                        # The removed session can attach nothing more, so its
                        # generation entry is pure table growth past this point;
                        # dropped here so the fence table stays bounded by the
                        # number of live parents.
                        forget_generation(key)
                    self._release_turn_ceiling(key, requested_key)
