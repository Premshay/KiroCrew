...
def _turn_counter_for(session_key: str) -> int | None:
    ...
    """
    try:
        state = cast("DashboardState | None", get_dashboard_state())
        if state is None:
            return None
...
async def handle_message(
    ...
    *channel_agent* overrides the default agent for this channel (set via
    ...
    _pre_footer_total = _turn_counter_for(session_key)
    ...
    if options and _footer_ts:
        ...
        # reports as idle.
        _superseded = sessions.is_busy(session_key) or (
            _turn_counter_for(session_key) != _pre_footer_total
        )
        if _superseded:
            try:
                await expire_slack_options(
