...
class SpawnAdmissionCoordinator(ManagerComponent):
    ...
    def spawn_impl(
        ...
        # Per-parent active-child cap (``agent.max_per_parent`` and its
        # per-agent overrides) on top of the global stagger/concurrency gate:
        # one parent must not hold every slot, so a parent already at its cap
        # QUEUES rather than starting, and its members drain as its own
        # children finish. Folded in here rather than into
        # ``_should_stagger_queue`` because only this call site knows the
        # parent and the resolved agent the cap is keyed on.
        parent_at_capacity = self._manager._parent_at_capacity(parent_session_key, agent)
    ...
    def _drain_queue_impl(self) -> None:
        ...
        if elapsed < self._manager._spawn_stagger_secs:
            ...
            try:
                ...
                pass  # no running loop (sync/test context)
            return
        # Skip past members whose parent is still at its cap instead of
        # popping the head unconditionally: a saturated parent at the front
        # would otherwise block every other parent's queued work behind it,
        # and this pump only runs once per stagger interval.
        queue_index = next(
            (
                index
                for index, queued in enumerate(self._manager._queue)
                if not self._manager._parent_at_capacity(
        ...
        if queue_index is None:
            return
        params = self._manager._queue.pop(queue_index)
