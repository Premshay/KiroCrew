...
class _GateMixin(ManagerComponent):
    ...
    def spawn_impl(
        ...
        *,
        ...
        # Per-parent active-child cap (``agent.max_per_parent`` and its per-agent
        # overrides) on top of the global stagger/concurrency gate: one parent
        # must not hold every slot, so a parent already at its cap QUEUES rather
        # than starting. Upstream's lane dispatch spreads parents across the
        # window but enforces no hard per-parent cap, so this stays the fork's
        # own gate. It was a call site in the fork's ``admission.py``, which
        # upstream split into this package.
