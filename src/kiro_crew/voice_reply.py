...
- ``piper`` (recommended offline default): Local neural TTS via the ``piper``
  CLI (https://github.com/OHF-Voice/piper1-gpl, published as the ``piper-tts``
...
# fresh machine can always reach; Polly remains fully supported when explicitly
...
async def _run_tts_subprocess(
    ...
    *,
    ...
    ``extra_env`` is overlaid on the scrubbed environment rather than on
    ``os.environ``: the Pocket engine selects itself and its voice by
    environment variable, and it must not buy that selection back by widening
    ...
    try:
        ...
        if extra_env:
            child_env = {**(child_env or {}), **extra_env}
...
async def _synthesize_piper(
    ...
    try:
        ...
        succeeded = await _run_tts_subprocess(
            ...
            display="Piper TTS",
            stdin_text=text,
            # Pocket runs through this same binary and selects its engine/voice
            # by environment, so the overlay travels with the spawn.
...
def synthesis_settings(raw: dict | None) -> dict:
    """The shared ``voice_reply`` section as :func:`synthesize_and_deliver` kwargs.
