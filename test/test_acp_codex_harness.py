...
def _effort_provider(backend: str, model: str) -> AcpProvider:
    ...
    # The push addresses the option id the session advertised for effort
    # (``effort`` here; codex builds can spell it ``reasoning_effort``), so the
    # double has to answer that question the way a live client does.
    provider._client.effort_config_option_id = MagicMock(
        return_value=effort_config_option_id(backend)
    )
    return provider
