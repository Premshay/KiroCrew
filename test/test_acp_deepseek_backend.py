...
@pytest.mark.asyncio
@pytest.mark.parametrize("method", ["session/new", "session/load", "session/resume"])
async def test_a_stalled_restore_keeps_its_mcp_detail(tmp_path, method) -> None:
    """Every session bind timeout retains the expected MCP server names."""
    from kiro_crew.acp.client import AcpTimeoutError

    client = _client(tmp_path, ACP_BACKEND_DEEPSEEK)
    with pytest.raises(AcpTimeoutError, match="test-server"):
        await client._wait_for_response(
            1, timeout=0, method=method, expected_mcp=[{"name": "test-server"}]
        )
...
@pytest.mark.parametrize("option_id", ["effort", "reasoning_effort"])
def test_the_levels_parser_reads_the_table(tmp_path, option_id) -> None:
    """The advertised selector overrides the backend's default spelling."""
    client = _client(tmp_path, ACP_BACKEND_DEEPSEEK)
    client._acp_config_options = [
        {"id": option_id, "options": [{"value": "low"}, {"value": "high"}]}
    ]
    assert client.effort_config_option_id() == option_id
    assert client.get_valid_effort_levels() == ["low", "high"]


@pytest.mark.asyncio
async def test_no_site_that_asks_about_effort_spells_the_id_itself(tmp_path) -> None:
    """A missing advertised selector prevents an unsupported config write."""
    from kiro_crew.providers.acp import AcpProvider

    provider = AcpProvider.__new__(AcpProvider)
    provider._client = _client(tmp_path, ACP_BACKEND_DEEPSEEK)
