...
def test_kas_capabilities_open_only_the_settings_channel():
    ...
    # The shared set carries a ``_meta`` of its own (claude-agent-acp's
    # subagent-transcript opt-in), which KAS's envelope replaces rather than
    # extends -- so the comparison is on everything outside ``_meta``, both sides.
