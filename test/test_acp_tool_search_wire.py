...
class TestEveryKasCapableConstructorThreadsTheSetting:
    ...
    async def test_the_background_runtime_is_built_with_the_setting(self, enabled):
        from kiro_crew.config import KiroCrewConfig
        from kiro_crew.session import BACKGROUND_AGENT, SessionManager

        cfg = KiroCrewConfig()
        cfg.agent.acp_backend = ACP_BACKEND_KAS
        cfg.agent.tool_search = enabled
        cfg.agent.tool_search_min_pct = 12
        cfg.agent.tool_search_min_tokens = 4000
        manager = SessionManager(cfg)
        runtime = MagicMock()
        runtime.spawn = AsyncMock()
        runtime.create_session = AsyncMock(return_value=object())
        runtime.kill = AsyncMock()
        binding = {"model": "bound-model", "model_switch_method": "session_set_model"}
        with (
            patch("kiro_crew.acp.runtime.AcpRuntime", return_value=runtime) as constructor,
            patch("kiro_crew.session.runtime_client_binding", return_value=binding) as resolve,
        ):
            try:
                handle = await manager.get_bg_session()
                resolve.assert_called_once_with(BACKGROUND_AGENT)
                assert constructor.call_args.kwargs == {
                    ...
                    "tool_search": ToolSearchSettings(enabled, 12, 4000),
                    **binding,
                }
                runtime.spawn.assert_awaited_once()
                runtime.create_session.assert_awaited_once_with(agent=BACKGROUND_AGENT)
                assert handle is runtime.create_session.return_value
            finally:
