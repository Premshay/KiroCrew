...
class TestStopTurnSlotState:
    ...
    async def test_interrupt_idle_dispatches_queue_when_no_successor(self, tmp_path, monkeypatch):
        ...
        state = self._make_state(tmp_path, monkeypatch)
        slot = state.get_or_create_slot("s1")
        # Mirror the idle _stage_loop: cancellation skips its queue handoff and
        # its exit clears slot.task, so the handler must recognise the
        # preserved queue from the cleared slot alone.
        ...
        try:
            ...
            assert slot._stop_state == "idle"
            assert runner_task.cancelled()
            # The cancelled loop's exit cleared slot.task (no successor claimed
            # it), so the handler dispatches the preserved queue itself.
            assert slot.task is None
