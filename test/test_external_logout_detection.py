...
class _FakeProvider:
    def __init__(self, backend: str, *, sid: str = "") -> None:
        ...
        self.session_id = sid
        self.session_provider_label = "acp"
        self.cwd = "/test-workspace"
...
class TestRetirementCoverage:
    ...
    @pytest.mark.asyncio
    async def test_streamed_sid_cannot_undo_identity_retirement(self) -> None:
        smap = self._manager()
        provider = _FakeProvider("", sid="sid-old-account")
        smap._sessions["marked"] = self._session(provider, busy=True)
        smap._session_map.set("marked", "sid-old-account")

        await smap.retire_kiro_identity_sessions()
        assert smap._sessions["marked"].retire_on_identity_change is True
        assert self._stored_sid(smap, "marked") == ""

        await smap.persist_provider_session("marked", provider)
    ...
    async def test_late_streamed_sid_requires_the_registered_provider(self, replacement) -> None:
        smap = self._manager()
        stale = _FakeProvider("", sid="sid-stale")
        if replacement:
            current = _FakeProvider("", sid="sid-current")
            smap._sessions["key"] = self._session(current)
            smap._session_map.set("key", "sid-current")

        await smap.persist_provider_session("key", stale)

        assert self._stored_sid(smap, "key") == ("sid-current" if replacement else "")
    ...
    def test_every_sid_writer_is_registration_or_identity_fenced(self) -> None:
        ...
        assert len(writers) >= 3, f"session-map writer scan found only {writers}"
        assert (
            not offenders
