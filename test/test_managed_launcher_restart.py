...
def _state():
    return SimpleNamespace(
        ...
        sessions=SimpleNamespace(
            close_all=AsyncMock(), restart_barrier_snapshot=AsyncMock(return_value=[])
        ),
        restart_barrier=RestartBarrier(),
        _slots={},
        broadcast_ws=Mock(),
    )
...
class TestLauncherContract:
    ...
    async def test_bad_target_rejected_before_drain(self, monkeypatch, tmp_path, kind):
        ...
        result = await updates._restart_gateway(state, resolver=fallback)
        assert result == updates.GatewayRestartResult(restarted=False)
...
if os.environ.pop("PROBE_FIRST", ""):
    ...
    state = SimpleNamespace(_gateway_restart_in_progress=False,
        ...
        sessions=SimpleNamespace(close_all=AsyncMock(),
            restart_barrier_snapshot=AsyncMock(return_value=[])),
        restart_barrier=RestartBarrier(), _slots={},
        broadcast_ws=lambda *a: None)
