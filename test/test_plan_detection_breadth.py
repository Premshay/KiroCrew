...
class TestCaseIsNotPartOfThePlanShape:
    def test_lowercase_bold_list_is_a_plan(self):
        """The IGNORECASE fix is preserved; the FORK's stage requirement refuses
        a bold-only list whatever its case. """
        text = (
    ...
    def test_mixed_case_bold_list_is_a_plan(self):
        """Case consistency is irrelevant; a bold-only list has no stage, so the
        fork refuses it. """
...
class TestSubstepsDoNotBreakTheStageRun:
    ...
    def test_substeps_alone_under_one_stage_are_not_enough(self):
        """FORK DIVERGENCE: the fork scores stage_run + bold_run, so one stage
        plus its two substeps IS a plan (pinned by the fork's own
        test_looks_like_plan_true_numbered_bold_with_stage). """
...
class TestBothShapesFeedOneRun:
    def test_stage_line_then_bold_item_is_a_two_stage_plan(self):
        """FORK DIVERGENCE: the bold item numbered 2 does not start a run from
        one, so the fork's sum (1 + 0) refuses it. """
        text = "Stage 1: Survey the callers\n2. **Patch** the helper\n"
        assert looks_like_plan(text) is False

    def test_bold_item_then_stage_line_is_a_two_stage_plan(self):
        """FORK DIVERGENCE: the stage line numbered 2 does not start a run from
        one, so the fork's sum (1 + 0) refuses it. """
    ...
    def test_two_shapes_repeating_one_number_is_not_a_run(self):
        """FORK DIVERGENCE: the fork sums the runs, so a stage line plus a bold
        item numbered 1 (1 + 1) qualifies. """
        assert looks_like_plan("Stage 1: Survey\n1. **Survey** the callers") is True
