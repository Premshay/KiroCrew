...
class TestPushGuardCredentialRedaction:
    ...
    def test_fetch_error_redacts_credential_at_truncation_boundary(self, tmp_path):
        ...
        # The redacted form should be present (proves redaction happened).
        assert (
            "<redacted>" in combined
        ), "No <redacted> marker found — redaction may not have run at all"
...
class TestPushGuardScpCredentialRedaction:
    ...
    def fake_ssh(self, tmp_path):
        """Create a fake SSH that fails immediately echoing the remote verbatim."""
        ssh_script = tmp_path / "fake_ssh"
        # The script echoes the full SSH command line's host arg to stderr,
        # simulating what real ssh does on auth failure.  We use $@ to capture
        # the arguments git passes (which include user@host).
        ssh_script.write_text(
            "#!/bin/sh\n"
            'echo "$@" | grep -o "[^[:space:]]*@[^[:space:]]*" >&2\n'
            'echo "Permission denied (publickey)." >&2\n'
            "exit 255\n"
        )
        ssh_script.chmod(0o755)
        return str(ssh_script)
    ...
    def test_scp_permission_denied_redacts_userinfo(self, tmp_path, fake_ssh):
        ...
        assert "<redacted>@git.example.com" in combined, (
            "Expected exact redacted form '<redacted>@git.example.com' not "
...
class TestRedactCredentialsUnit:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            # --- scp-style (user@host:path) — pass 3 redacts userinfo ---
            ("git@github.com:org/repo.git", "<redacted>@github.com:org/repo.git"),
            # SCP with credential-like token in userinfo position
            (
                "tok_secret123@git.example.com:team/Repo.git",
                "<redacted>@git.example.com:team/Repo.git",
            ),
            # SCP in SSH permission-denied stderr context
            (
                "tok_secret123@git.example.com: Permission denied (publickey).",
                "<redacted>@git.example.com: Permission denied (publickey).",
            ),
            # SCP after whitespace (as in "fatal: Could not read from remote")
            # Note: the quote preceding the userinfo is consumed by group 1
            # (it's part of [^\s/@:]+) so it becomes part of <redacted>.
            (
                "fatal: Could not read from remote 'deploy_tok@git.example.com:org/r.git'",
                "fatal: Could not read from remote <redacted>@git.example.com:org/r.git'",
            ),
            # SCP after double-quote (git wraps remotes in quotes);
            # the opening quote is consumed by group 1 — acceptable over-redaction.
            (
                'host "secret_user@git.example.com" port 22',
                'host <redacted>@git.example.com" port 22',
            ),
        ],
        ids=lambda v: v[:50],
    )
