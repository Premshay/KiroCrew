...
class TestOpenCodeBackendResolvesTheTool:
    ...
    def test_every_directive_tool_resolves_from_the_opencode_title(self, tool):
        ...
        # ``trusted=False``: opencode emits no ``_meta.kiro`` pair, so the frame
        # carries no trusted identity -- and the title must resolve without one,
        # which is what makes it a selector rather than a grant.
        assert session_directive.directive_tool_from_call("", "", title, trusted=False) == tool
    ...
    def test_a_lookalike_single_underscore_name_resolves_to_nothing(self, title):
        # Asked with trust granted too, so a refusal here is the title check's own
        # and not a side effect of an untrusted frame.
        for trusted in (False, True):
            assert (
                session_directive.directive_tool_from_call("", "", title, trusted=trusted) == ""
            )
