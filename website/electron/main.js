...
async function showLoadingThenConnect(win, backendUrl = BACKEND_URL) {
  ...
  try {
    ...
    for (let attempt = 0; ; attempt++) {
      ...
      if (win.isDestroyed()) return;
      // token-prompt.html loads into THIS (the main) window. If it is in
      // fullscreen/kiosk the traffic lights + app menu are hidden and the page
      // has no in-page exit, trapping the user (force-kill was the only way
      // out). Drop immersive modes first so Close / Cmd-Q work.
