...
const CANCELLED = (): Flagged =>
  Object.assign(new Error("cancelled"), { cancelled: true });
const TIMEOUT = (): Flagged =>
  Object.assign(new Error("still running"), { timeout: true });

export default function DesignCritiquePage() {
  const [queryClient] = useState(() => new QueryClient());
  return (
    <QueryClientProvider client={queryClient}>
      <DesignCritiquePageContent />
    </QueryClientProvider>
  );
}

function DesignCritiquePageContent() {
  ...
  const { data: agentList } = useQuery({
    ...
    enabled: typeof designCritiqueApi.listAgents === "function",
  });
  const { data: reviewRunData } = useQuery({
    queryKey: ["design-critique-runs"],
    queryFn: designCritiqueApi.listReviewRuns,
    enabled: typeof designCritiqueApi.listReviewRuns === "function",
  });
  const { data: projectContextData } = useQuery({
    ...
    enabled: typeof designCritiqueApi.listProjectContexts === "function",
  });
  const { data: designRoundData } = useQuery({
    queryKey: ["design-critique-design-rounds"],
    queryFn: designCritiqueApi.listDesignRounds,
    enabled: typeof designCritiqueApi.listDesignRounds === "function",
  });
  const agents = agentList?.agents || [];
  const projectContexts = projectContextData?.contexts || [];
  const activeAgent =
    agents.length && !agents.some((agent) => agent.name === selectedAgent)
      ? DEFAULT_AGENT
      : selectedAgent;

  useEffect(() => {
    saveSelectedAgent(activeAgent);
  }, [activeAgent]);
  useEffect(() => {
    saveReviewBrief(reviewBrief);
  }, [reviewBrief]);
  ...
  const saveProjectContext = async () => {
    ...
    try {
      const response = reviewBrief.contextId
        ? await designCritiqueApi.updateProjectContext(
            reviewBrief.contextId,
            input,
          )
      ...
      setErr(
        error instanceof Error
          ? error.message
          : i18nT(
              "apps.designCritique.scopeBuilder.project_context_request_failed",
            ),
      );
    } finally {
  ...
  const prepareDesignRound = async (input: {
    ...
    try {
      ...
      setErr(
        error instanceof Error
          ? error.message
          : "Could not prepare the Claude Design round.",
      );
    } finally {
  ...
  const updateDesignRound = async (
    roundId: string,
    update: Partial<DesignRound>,
  ) => {
    ...
    try {
      ...
      setErr(
        error instanceof Error
          ? error.message
          : "Could not update the Claude Design round.",
      );
    } finally {
      setDesignRoundBusy(false);
    }
  ...
  useEffect(() => {
    const restored = (reviewRunData?.runs || []).filter(
      (run) => run.status === "completed" && run.report,
    );
    if (!restored.length) return;
    const current = loadHistory();
    const next = restored.reduce<HistoryEntry[]>((entries, run) => {
      if (entries.some((entry) => entry.slotKey === run.slot_key))
        return entries;
      const screens = Array.isArray(run.screens)
        ? run.screens.filter(
            (screen): screen is Screen =>
              !!screen &&
              typeof screen.url === "string" &&
              typeof screen.label === "string" &&
              typeof screen.step === "number",
          )
        : [];
      ...
      return [
        {
          ...
          runId: run.id,
          screens,
          thumbUrl: screens[0]?.url || "",
          read: report.overallRead || "",
          report,
        },
        ...entries,
      ];
  ...
  const isCancelled = (slotKey: string): boolean =>
    cancelledRef.current.has(slotKey);
  ...
  useEffect(() => {
    if (phase !== "uploading" && phase !== "analyzing" && phase !== "scanning")
      return;
    const tick = () => {
  ...
  const pollForReport = async <T,>(slotKey: string): Promise<T> => {
    ...
    while (Date.now() - began < HARD_CAP_MS) {
      ...
      let d;
      try {
        d = await designCritiqueApi.getSlot(slotKey);
      } catch {
        if (++misses >= 8)
          throw new Error(
            "That run is no longer available — start a new critique.",
          );
        continue;
      }
      misses = 0;
      ...
      if (d && !d.running && c) {
        const p = jsonFromMessages<T>(d.messages, looksLikeReport);
        if (p) return p;
        throw new Error("The critic replied but not in a readable format.");
      }
    }
    throw TIMEOUT();
  };
  ...
  const pollForText = async (slotKey: string): Promise<string> => {
    ...
    while (Date.now() - began < 3 * 60 * 1000) {
      ...
      try {
        ...
        if (++misses >= 8) throw new Error("lost contact");
        continue;
      }
      misses = 0;
      const c = lastAssistant(d && d.messages);
  ...
  const startClock = (fromMs?: number) => {
    startedAtRef.current = fromMs || Date.now();
    setElapsed(0);
  };
  ...
  const dropSlot = (slotKey: string) => {
    ...
    untrackSlot(slotKey);
    unmarkLive(slotKey);
    designCritiqueApi.deleteSlot(slotKey);
  };
  ...
  const createReviewRun = async (
    ...
    if (typeof designCritiqueApi.createReviewRun !== "function") return "";
    const model =
      typeof designCritiqueApi.resolveAgentModel === "function"
        ? await designCritiqueApi
  ...
  const updateReviewRun = (
    ...
    update: Partial<ReviewRun>,
  ): Promise<void> =>
    runId && typeof designCritiqueApi.updateReviewRun === "function"
      ? designCritiqueApi.updateReviewRun(runId, update).then(() => undefined)
      : Promise.resolve();
  ...
  const showReport = (raw: Report, screens: Screen[], entry?: HistoryEntry) => {
    ...
    if (askSlotRef.current) {
      dropSlot(askSlotRef.current);
      askSlotRef.current = "";
      askAgentRef.current = DEFAULT_AGENT;
    }
    setOpen(new Set([0]));
    setActive(null);
    setZoom(false);
  ...
  const finishReport = async (
    ...
    if (waiting)
      showReport(
        rep,
        screens,
        next.find((e) => e.slotKey === slotKey) || next[0],
      );
    else
      setJustFinished({
        slotKey,
        read: rep.overallRead || "Critique ready",
        screens,
        report: rep,
      });
  };
  ...
  const ask = async (prompt: string, uploaded: Screen[], foreground = true) => {
    ...
    try {
      ...
      runId = await createReviewRun(
        ...
        agent,
        "analyzing",
        { kind: "screenshots", brief: briefForRun() },
        uploaded,
      );
      ...
      await failWith(e, slotKey, runId);
    }
  };
  ...
  const runImages = async (fileList: File[]) => {
    ...
    try {
      ...
      if (mine) {
        setCurrent({ report: null, screens: uploaded });
        setScreenIdx(0);
        setPhase("analyzing");
      }
      await ask(IMAGES_PROMPT(paths, briefForRun(), method), uploaded, mine);
    } catch (e) {
  ...
  const applyDiscovery = (
    ...
    const list = Array.isArray(info.screens)
      ? info.screens.filter((s) => s && s.id)
      : [];
    if (!list.length) {
      clearJob(jobKey);
      setErr(
        discoveryNote(info) ||
          i18nT(
            "apps.designCritique.designCritiquePage.i_got_in_but_there_s_nothing_in_there_i_can",
          ),
      );
      setPhase("error");
      return;
    }
    const norm = normalizeScope({ ...info, screens: list }) || {
      ...info,
      screens: list,
      flows: [],
    };
    ...
    const preset =
      first && first.screenIds && first.screenIds.length
  ...
  const runRef = async (raw: string) => {
    ...
    startClock();
    setWriting(false);
    setPendingKind(det.kind);
    setCurrent({ report: null, screens: [] });
    setScreenIdx(0);
    setScope(null);
    setPicked([]);
    setRefHandle("");
    setRefTarget({ kind: det.kind, value: det.value });
    setSlot(jobKey);
    ...
    setPhase("scanning");
    try {
      const info = await designCritiqueApi.discover(
        det.kind,
        det.value,
        (jobId) => {
      ...
      setErr(
        e instanceof Error
          ? e.message
          : i18nT(
              "apps.designCritique.designCritiquePage.that_scan_didn_t_finish",
            ),
      );
      setPhase("error");
    }
  ...
  const critiqueRendered = async (
    ...
    const rendered = Array.isArray(out.screens)
      ? out.screens.filter((s) => s && s.path)
      : [];
    if (!rendered.length) {
      if (jobKey) {
        clearJob(jobKey);
    ...
    const uploaded = rendered.map((s, i) => ({
      step: i + 1,
      label: s.label,
      url: fileUrl(s.path),
    }));
  ...
  const runScoped = async () => {
    ...
    const picks = picked
      .map((id) => byId.get(id))
      .filter(Boolean) as DiscoveryScreen[];
    const jobKey = slot;
    const seq = ++runSeqRef.current;
  ...
  useEffect(() => {
    ...
    if (job.stage === "scoping" && job.scope) {
      ...
      if (job.kind && job.value)
        setRefTarget({ kind: job.kind, value: job.value });
      setCurrent({ report: null, screens: [] });
      setPhase("scoping");
      return;
    }
    ...
    if (job.stage === "scanning") {
      ...
      if (job.discoverJob) {
        ...
        (async () => {
          try {
            const info = await designCritiqueApi.pollDiscover(
              job.discoverJob as string,
            );
            applyDiscovery(
              info,
              { kind: job.kind || "", value: job.value || "" },
              job.slotKey,
            );
          } catch (e) {
            const flag = e as Flagged;
    ...
    if (job.stage === "rendering" && job.renderJob) {
      ...
      (async () => {
        try {
          const out = await designCritiqueApi.pollRender(
            job.renderJob as string,
          );
          await critiqueRendered(
            out,
            job.slotKey,
            undefined,
            job.refBrief || "",
    ...
    (async () => {
      try {
        ...
        await updateReviewRun(job.runId, {
          status: "failed",
          error: { code: e instanceof Error ? e.message : "unknown_failure" },
        }).catch(() => undefined);
        endRun(job.slotKey);
  ...
  const busy =
    phase === "uploading" || phase === "analyzing" || phase === "scanning";

  const report = current?.report ?? null;
  const screens = (current && current.screens) || [];
  const isFlow = screens.length > 1;
  const shown =
    screens[Math.min(screenIdx, Math.max(0, screens.length - 1))] || null;
  ...
  const moveStaged = (i: number, dir: number) =>
    setStaged((prev) => {
      const j = i + dir;
  ...
  const discoveryNote = (info: Partial<Scope>): string => {
    ...
    if (!sc) return "";
    return (
      (sc.cannotSee && sc.cannotSee.length ? sc.cannotSee.join(" ") : "") ||
      sc.note ||
      ""
    );
  };
  ...
  const onDrop = (e: React.DragEvent) => {
    e.preventDefault();
    setDragging(false);
    if (!busy) addFiles(e.dataTransfer && e.dataTransfer.files);
  };
  ...
  const captureSelection = () => {
    ...
    if (!w || w.isCollapsed || !w.rangeCount) {
      setSel(null);
      return;
    }
    const quote = String(w.toString()).replace(/\s+/g, " ").trim();
    if (quote.length < 4 || quote.length > 320) {
      setSel(null);
      return;
    }
    ...
    setAsks((prev) =>
      prev.map((a) =>
        a.id === askId
          ? {
              ...a,
              turns: a.turns.concat([
                { t: stamp, q: question || "", a: "", pending: true },
              ]),
            }
          : a,
      ),
    ...
    try {
      ...
      await send(
        ...
        seedWith
          ...
          : "Follow-up on the same highlighted text: " +
              (question || "Say more.") +
        ...
        const next = prev.map((a) =>
          a.id === askId
            ? {
                ...a,
                turns: a.turns.map((t) =>
                  t.t === stamp ? { ...t, a: text, pending: false } : t,
                ),
              }
            : a,
        );
        ...
        prev.map((a) =>
          a.id === askId
            ? {
                ...
                turns: a.turns.map((t) =>
                  t.t === stamp
                    ? { ...t, a: msg, pending: false, failed: true }
                    : t,
                ),
              }
            : a,
  ...
  const cancelRun = async () => {
    ...
    if (k) {
      void updateReviewRun(slotRunId, {
        status: "interrupted",
        error: { code: "cancelled" },
      }).catch(() => undefined);
      cancelledRef.current.add(k);
      endRun(k);
    }
    activeSlotRef.current = "";
    setSlot("");
  ...
  const newCritique = () => {
    ...
    if (running) {
      ...
      if (k) {
        ...
        setCritiques(
          beginPendingCritique(k, (current && current.screens) || []),
        );
      } else if (slot) {
    ...
    if (!running && slot) {
      ...
      setTimeout(() => {
        cancelledRef.current.delete(slot);
      }, 2500);
      endRun(slot);
    }
    if (!running) {
  ...
  const openExample = () => {
    setMenuOpen(false);
    showReport(SAMPLE_REPORT, SAMPLE_SCREENS);
  };
  ...
  const selectCritique = (e: HistoryEntry) => {
    ...
    const scr =
      e.screens && e.screens.length
        ? e.screens
        : e.thumbUrl
          ? [{ step: 1, label: "Screen 1", url: e.thumbUrl }]
          : [];
  ...
  const togglePick = (id: string) =>
    setPicked((p) =>
      p.includes(id) ? p.filter((x) => x !== id) : p.concat([id]),
    );
  const dropPickAt = (id: string, overId: string) =>
    setPicked((p) => {
  ...
  const movePick = (id: string, dir: number) =>
    setPicked((p) => {
  ...
  const useFlow = (f: Flow) =>
    setPicked(
      (f.screenIds || []).filter((id) =>
        scope ? scope.screens.some((s) => s.id === id) : false,
      ),
    );

  // ── derived finding structures ────────────────────────────────────────────
  const bySev = (a: Finding, b: Finding) =>
    sevOf(a.severity).rank - sevOf(b.severity).rank;
  const all: Finding[] =
    report && Array.isArray(report.findings) ? report.findings : [];
  const stepOf = (f: Finding) =>
    Array.isArray(f.steps) && f.steps.length ? f.steps[0] : 1;
  const isFlowFinding = (f: Finding) =>
    f.scope === "flow" || (Array.isArray(f.steps) && f.steps.length > 1);
  const flowFindings = isFlow ? all.filter(isFlowFinding).sort(bySev) : [];
  const screenFindings = all
    .filter((f) => !isFlow || !isFlowFinding(f))
    .sort(bySev);
  const designRounds = (designRoundData?.rounds || []).filter(
    (designRound) => !!slotRunId && designRound.review_run_id === slotRunId,
  );
  ...
  const stepRange = (f: Finding) => {
    ...
    // really is an array.
    const st = Array.isArray(f.steps)
      ? f.steps.slice().sort((a, b) => a - b)
      : [];
  ...
  const buildMarkers = (interactive: boolean): ReactNode[] =>
    screenFindings
        ...
        return (
          ...
          <span
            ...
            style={{
              ...
              boxShadow: on
                ? "0 0 0 3px rgba(0,0,0,.5), 0 0 0 11px color-mix(in srgb, " +
                  s.color +
                  " 22%, transparent)"
  ...
  let chips: ReactNode[] = (
    ["catastrophe", "major", "minor", "cosmetic"] as const
  )
    .filter((k) => (t[k] || 0) > 0)
    .map((k) => {
      const s = sevOf(k);
      const Icon = s.icon;
      return (
        <span key={k} style={{ ...S.chip, color: s.color }}>
          <Icon size={12} />
          {(t[k] || 0) + " " + s.label.toLowerCase()}
        </span>
      );
    });
  if (report && !chips.length)
    chips = [
      <span key="ok" style={{ ...S.chip, color: "#3fae6b" }}>
        <Check size={12} />
        {i18nT("apps.designCritique.designCritiquePage.nothing_major")}
      </span>,
    ];
  ...
  const entryMeta = (e: HistoryEntry) => {
    ...
    return (
      (n > 1 ? n + " screens · " : n === 1 ? "1 screen · " : "") + relTime(e.ts)
    );
  };
  ...
  const railHead = (
    <div style={S.railHead}>
      <div style={S.title}>
        <PencilRuler size={19} style={{ color: "var(--accent)" }} />
        {i18nT("apps.designCritique.designCritiquePage.design_critique")}
      </div>
      <div style={S.railCtrls}>
        {phase !== "new" ? (
          <button
            ...
            onClick={newCritique}
            title={i18nT(
              "apps.designCritique.designCritiquePage.start_a_new_critique_anything_already_running_ke",
            )}
          >
            <Plus size={13} />
            {i18nT("apps.designCritique.designCritiquePage.new")}
          </button>
        ) : null}
        {busy ? (
          <button
            ...
            onClick={() => {
              setJustFinished(null);
              setPhase(
                pendingKind && !screens.length ? "scanning" : "analyzing",
              );
            }}
            title={i18nT(
              "apps.designCritique.designCritiquePage.a_critique_is_still_running_click_to_watch_it",
            )}
          >
            <Spinner size={12} reduceMotion={reduceMotion} />
            {i18nT("apps.designCritique.designCritiquePage.running")}
          </button>
        ) : null}
        {!busy && justFinished ? (
          <button
            style={{ ...S.runChip, ...S.readyChip }}
            onClick={() => {
              const h = loadHistory();
              const mine = h.find((e) => e.slotKey === justFinished.slotKey);
              if (mine)
                showReport(justFinished.report, justFinished.screens, mine);
              setJustFinished(null);
            }}
            title={justFinished.read}
          >
            <Check size={12} />
            {i18nT("apps.designCritique.designCritiquePage.critique_ready")}
          </button>
        ) : null}
        {phase !== "new" && critiques.length ? (
          <div style={{ position: "relative" }}>
            <button
              ...
              title={i18nT(
                "apps.designCritique.designCritiquePage.switch_critique",
              )}
            >
            ...
            {menuOpen ? (
              <div style={S.menu}>
                {critiques.map((e) => (
                  ...
                  >
                    {e.thumbUrl ? (
                      <img src={e.thumbUrl} style={S.menuThumb} alt="" />
                    ) : null}
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div
                        style={{
                          ...S.menuRead,
                          ...(e.pending ? S.pendingRead : {}),
                        }}
                      >
                        {e.pending
                          ? i18nT(
                              "apps.designCritique.designCritiquePage.running",
                            )
                          : e.read || "Critique"}
                      </div>
    ...
    for (const hit of hits) {
      ...
      out.push(
        <span
          ...
          tabIndex={0}
          title={i18nT(
            "apps.designCritique.designCritiquePage.your_question_click_to_see_the_answer",
          )}
          onClick={(e) => {
            e.stopPropagation();
  ...
  const renderRow = (f: Finding) => {
    ...
    return (
      <FindingRow
        ...
        pinNum={pinNo.get(f)}
        stepRange={stepRange(f)}
        withMarks={withMarks}
        onEnter={() => setActive(i)}
        onLeave={() => setActive(null)}
        onClick={() => {
          toggle(i);
          if (!flowScoped) showStepOf(f);
        }}
        rowRef={(el) => {
  ...
  if (phase === "report" && report) {
          ...
          flowFindings.length ? (
            <div key="flow">
              <div style={S.sectionH}>
                {i18nT(
                  "apps.designCritique.designCritiquePage.across_the_flow",
                )}
          ...
          ...screens.map((sc) => {
            ...
            return (
              <div key={"st" + sc.step}>
                <Clickable
                  style={S.stepH}
                  onClick={() => setScreenIdx(screens.indexOf(sc))}
                  title={i18nT(
                    "apps.designCritique.designCritiquePage.show_this_screen",
                  )}
                >
                  <span>{"Step " + sc.step + " · "}</span>
                  <span style={S.stepName} title={sc.label}>
                    {shortLabel(sc.label)}
                  </span>
                  {onScreen ? (
                    <span style={{ ...S.stepCount, color: "var(--accent)" }}>
                      {i18nT(
                        "apps.designCritique.designCritiquePage.on_screen",
                      )}
                    </span>
                  ) : (
                    <span style={{ ...S.stepCount, color: "var(--muted)" }}>
                      {mine.length
                        ? mine.length +
      ...
      : screenFindings.length
        ? [
            <div key="wf">
              <div style={S.sectionH}>
                {i18nT(
                  "apps.designCritique.designCritiquePage.what_i_d_tighten",
                )}
              </div>
              {screenFindings.map(renderRow)}
            </div>,
          ]
        : [];
    ...
    railBody = (
      <>
        <p key="read" style={S.readLine}>
          {withMarks(report.overallRead || "Design critique")}
        </p>
        {report.health || isFlow ? (
          <div key="hh" style={S.health}>
            {[report.health, isFlow ? " · " + screens.length + " screens" : ""]
              .filter(Boolean)
              .join("")}
          </div>
        ) : null}
        <div key="chips" style={S.chips}>
          {chips}
        </div>
        {Array.isArray(report.keep) && report.keep.length ? (
          <div key="keep">
            <div style={S.sectionH}>
              {i18nT("apps.designCritique.designCritiquePage.what_s_working")}
            </div>
            {report.keep.map((k, i) => (
              <div key={i} style={S.keepItem}>
                <Check
                  size={15}
                  style={{ color: "#3fae6b", flexShrink: 0, marginTop: "2px" }}
                />
                <span>{k}</span>
              </div>
            ))}
        ...
        {Array.isArray(report.couldNotSee) && report.couldNotSee.length ? (
          <div key="cns">
            <div style={S.sectionH}>
              {i18nT("apps.designCritique.designCritiquePage.couldn_t_see")}
            </div>
            {report.couldNotSee.map((c, i) => (
              <div key={i} style={S.seeItem}>
                {"· " + c}
              </div>
            ))}
            <div
            ...
            >
              <button style={S.linkBtn} onClick={sendScreenshots}>
                <Upload size={13} />
                {i18nT(
                  "apps.designCritique.designCritiquePage.send_screenshots",
                )}
              </button>
              <button style={S.linkBtn} onClick={critiqueRunning}>
                <ImageIcon size={13} />
                {i18nT(
                  "apps.designCritique.designCritiquePage.point_me_at_a_running_url",
                )}
              </button>
            </div>
          </div>
    ...
    railBody = (
      <>
        <p key="w" style={S.readLine}>
          {phase === "uploading" ? "Uploading…" : "Reading your design…"}
        </p>
        <div key="sub" style={S.health}>
          {screens.length > 1
            ? "A flow of " +
              screens.length +
              " screens — I’ll also check the jumps between them."
            : screens.length
              ? "One screen."
              : pendingKind
                ? "A " +
                  (KIND_LABEL[pendingKind] || "design") +
                  " — rendering it before judging it."
                : "Getting the screens ready."}
        </div>
        {screens.length ? (
          <div key="l">
            <div style={S.sectionH}>
              {screens.length > 1 ? "Screens in order" : "Screen"}
            </div>
            {screens.map((sc, i) => (
              <div
                key={i}
                style={{ ...S.seeItem, display: "flex", gap: "8px" }}
              >
                <span
                  style={{
                    color: "var(--muted)",
        ...
        <div key="nx">
          <div style={S.sectionH}>
            {i18nT("apps.designCritique.designCritiquePage.what_you_ll_get")}
          </div>
          {[
            "An overall read of how it’s doing",
            "What’s working, so it survives the next pass",
            "The top things to tighten, with pins on the screen",
          ].map((tx, i) => (
            <div key={i} style={S.seeItem}>
              {"· " + tx}
            </div>
    ...
    railBody = (
      <>
        <p key="sub" style={S.sub}>
          {i18nT(
            "apps.designCritique.designCritiquePage.point_me_at_a_screen_or_a_whole_flow_and_i_ll_re",
          )}
        </p>
        <button key="ex" style={S.linkBtn} onClick={openExample}>
          <Sparkle size={13} />
          {i18nT("apps.designCritique.designCritiquePage.see_an_example")}
        </button>
        <div key="h" style={S.sectionH}>
          {i18nT("apps.designCritique.designCritiquePage.your_critiques")}
        </div>
        {critiques.length ? (
          <div key="list">
            {critiques.map((e) => (
              ...
              >
                {e.thumbUrl ? (
                  <img src={e.thumbUrl} style={S.listThumb} alt="" />
                ) : null}
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div
                    style={{
          ...
          <div key="empty" style={S.emptyList}>
            {"No critiques yet." +
              (narrow
                ? " Drop a screenshot below."
                : " Drop a screenshot in the panel →")}
          </div>
        )}
      </>
    );
  ...
  if (phase === "scoping" && scope) {
    canvasInner = (
      <ScopingPicker
        ...
        dropPickAt={dropPickAt}
        movePick={movePick}
        useFlow={useFlow}
        setDragId={setDragId}
        setRefBrief={setRefBrief}
        runScoped={runScoped}
        onStartOver={newCritique}
      />
    );
  } else if (
    phase === "uploading" ||
              ...
              <div style={{ ...S.thumbBox, ...(on ? S.thumbBoxOn : {}) }}>
                ...
                {worst ? (
                  <span
                    style={{
                      ...S.thumbCount,
                      color: sevOf(worst.severity).color,
                    }}
                  >
                    <span
                      style={{
                        ...
                        height: "6px",
                        borderRadius: "999px",
                        background: sevOf(worst.severity).color,
                  ...
                  <span style={{ ...S.thumbCount, color: "#3fae6b" }}>
                    <Check size={11} />
                  </span>
                )}
              </div>
      ...
      <>
        ...
        <div style={S.previewWrap}>
          <Clickable
            ...
            title={i18nT("apps.designCritique.designCritiquePage.enlarge")}
          >
            <img
              src={thumbUrl}
              style={S.previewImg}
              alt={shown ? shown.label : "the design"}
            />
            {markers}
          </Clickable>
          <div style={S.pvbar}>
            ...
            {"Click to enlarge" +
              (markers.length
                ? " · numbered pins mark issues on this screen"
                : "") +
              (isFlow ? " · pick a step above" : "")}
          </div>
        </div>
      </>
    );
    ...
    canvasInner = (
      <div style={S.previewWrap}>
        <div style={{ ...S.muted, maxWidth: "420px", textAlign: "center" }}>
          {i18nT(
            "apps.designCritique.designCritiquePage.no_screens_to_show_the_critic_couldn_t_get_pixel",
          )}
        </div>
      </div>
    );
    ...
    canvasInner = (
      <Composer
        ...
        agents={agents}
        defaultAgent={agentList?.default_agent || DEFAULT_AGENT}
        ...
        brief={reviewBrief}
        setBrief={updateReviewBrief}
        selectContext={selectProjectContext}
        saveContext={saveProjectContext}
        deleteContext={deleteProjectContext}
        onPick={onPick}
        onDrop={onDrop}
    ...
    zoom && thumbUrl ? (
      ...
      >
        <div
          ...
          aria-modal="true"
          aria-label={i18nT(
            "apps.designCritique.designCritiquePage.full_size_view",
          )}
        >
          <img
            src={thumbUrl}
            style={S.lbImg}
            alt={i18nT("apps.designCritique.designCritiquePage.full_size")}
          />
          {buildMarkers(false)}
        </div>
        <button
  ...
  return (
    <div
      ref={rootRef}
      style={{ ...S.shell, ...(narrow ? { flexDirection: "column" } : {}) }}
    >
      {rail}
      {canvas}
      {lightbox}
