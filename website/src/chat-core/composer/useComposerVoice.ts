 ...
 *   - whether to offer hands-free ("car mode") dictation (`handsFree`), and how
 *     to send the turns it commits.
 *
...
export interface ComposerVoiceHost {
   ...
   * and listening re-arms (see useHandsFreeLoop). Omit and the host offers no
   * hands-free toggle — only the main chat opts in today.
   */
  handsFree?: {
    /** Send an endpointer-committed transcript as a turn. Bypasses the composer,
     *  so a draft the user left there stays untouched. */
    send: (text: string) => void
    /** The assistant holds the floor: a turn is running or its reply audio
     *  pipeline is busy. Store-derived, so the host supplies it. */
    assistantHoldsFloor: boolean
  }
}
...
export function useComposerVoice(host: ComposerVoiceHost) {
  ...
  const handsFreeSendRef = useRef(host.handsFree?.send); handsFreeSendRef.current = host.handsFree?.send
  ...
  // its auto-send flag. Assigned in render.
  ...
  const applyVoiceText = useCallback((text: string, transcriptSession: string | null, origin: TranscriptOrigin) => {
    ...
    // bypasses the composer entirely — no setInput race, and a draft the user
    // left in the composer stays untouched. A trivial transcript (noise) is
    // dropped; the loop re-arms either way via useHandsFreeLoop's cycle effect.
    if (handsFreeRef.current?.consumeAutoSend()) {
      ...
      if (target === sessionIdRef.current && handsFreeSendRef.current) {
        handsFreeRef.current.noteSent()
        handsFreeSendRef.current(trimmed)
        return
      }
      clearTurnMarks()
  ...
  // ---- hands-free ("car mode") loop ----------------------------------------
  // Batch-only by design: streaming providers ship their own semantic
  // endpointer (onEndpoint above), so the toggle is withheld when streaming is
  // active. Only a host that opts in (`handsFree`) offers it.
  const handsFreeHosted = !!host.handsFree
  const assistantHoldsFloor = !!host.handsFree?.assistantHoldsFloor
  const handsFreeUsable =
    handsFreeHosted && voiceInputSupported && sttConfigLoaded && sttEnabled && sttAvailable && !voice.streamEnabled
  const [handsFreePref, setHandsFreePref] = usePersistedBool(HANDS_FREE_LS_KEY, false)
  const handsFreePrefRef = useRef(handsFreePref); handsFreePrefRef.current = handsFreePref
  const handsFreeUsableRef = useRef(handsFreeUsable); handsFreeUsableRef.current = handsFreeUsable
  ...
  const { data: voiceCfg } = useQuery<{ autoSpeak?: boolean }>({
    queryKey: ['voiceConfig'],
    queryFn: () => api.voiceConfig(),
    enabled: handsFreeHosted,
  })
  const [autoSpeakOn, setAutoSpeakOn] = useState(false)
  useEffect(() => { if (voiceCfg) setAutoSpeakOn(!!voiceCfg.autoSpeak) }, [voiceCfg])
  ...
  useEffect(() => { if (!assistantHoldsFloor) setBargedIn(false) }, [assistantHoldsFloor])
  useEffect(() => { setBargedIn(false) }, [sessionId])
  const spokenConversation = autoSpeakOn || handsFreeSpeechOn
  const convoHold = spokenConversation && !bargedIn && assistantHoldsFloor
  const convoHoldRef = useRef(convoHold); convoHoldRef.current = convoHold
  const handsFree = useHandsFreeLoop({
    ...
    start: () => {
      if (!voiceRef.current.recording && !voiceRef.current.transcribing) toggleVoice()
    },
    stopCommit: () => {
  ...
  // Scopes reply audio to an armed headset conversation (useWebSocket keys the
  // sources, so co-mounted composers cannot mute each other).
  useEffect(() => {
    if (!handsFreeHosted) return
    setHandsFreeSpeechOn(handsFree.armed)
    window.dispatchEvent(new CustomEvent('handsfree-speech-changed', {
      detail: { source: instanceId, enabled: handsFree.armed },
    }))
    return () => {
      window.dispatchEvent(new CustomEvent('handsfree-speech-changed', {
        detail: { source: instanceId, enabled: false },
      }))
    }
  }, [handsFree.armed, instanceId, handsFreeHosted])
  ...
  return {
    ...
    sttDictationPanel,
    /** The hands-free loop (arm/disarm/exit), for a host that must end it on a
     *  manual act the atom cannot see — typing into the composer. */
    handsFree,
    handsFreeUsable,
    handsFreePref,
    handleVoiceToggle,
...
export function composerVoiceInputProps(cv: ComposerVoice) {
  ...
  return {
    ...
    /* Hands-free (fork): the mode toggle beside the mic and the loop-state strip. */
    handsFreeAvailable: cv.handsFreeUsable,
    handsFreeOn: cv.handsFreePref,
    onHandsFreeToggle: voiceInputSupported ? cv.handleHandsFreeToggle : undefined,
    handsFreePhase: cv.handsFree.phase,
    /* Routes through the hands-free loop when it is armed or preferred; a plain
       dictation toggle otherwise. */
    onVoiceToggle: voiceInputSupported ? cv.handleVoiceToggle : undefined,
