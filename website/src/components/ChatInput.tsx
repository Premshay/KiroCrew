...
import { ArrowUpFromLine, ArrowUp, Loader2, RotateCw, Plus, Crop, Bot, Mic, Keyboard, Square, BookOpen, X, ClipboardList, CheckCircle, Check, Ban, Sparkles, Target, Lock, Folder, FolderOpen, FileText, Headset, Volume2 } from 'lucide-react'
    ...
    <div className={`input-area px-4 pb-1 ${hasApproval ? 'pt-0' : 'pt-1'} mx-auto w-full flex flex-col`}
      ...
      >
        ...
        {handsFreePhase && (
          ...
          >
            {handsFreePhase === 'listening' && <><span className="relative flex h-2 w-2 shrink-0" aria-hidden="true"><span className="absolute inline-flex h-full w-full rounded-full bg-danger opacity-60 animate-ping" /><span className="relative inline-flex h-2 w-2 rounded-full bg-danger" /></span><span className="font-medium">{i18nT('components.chatInput.handsfree_listening')}</span></>}
            {handsFreePhase === 'processing' && <><Loader2 className="lucide-inline animate-spin shrink-0" /><span className="font-medium">{i18nT('components.chatInput.transcribing')}</span></>}
            {handsFreePhase === 'sent' && <><Check className="lucide-inline shrink-0" /><span className="font-medium">{i18nT('components.chatInput.handsfree_sent')}</span></>}
            {handsFreePhase === 'speaking' && <><Volume2 className="lucide-inline shrink-0" /><span className="font-medium">{i18nT('components.chatInput.handsfree_speaking')}</span></>}
          ...
          <div className="flex items-center gap-1 shrink-0">
            ...
            {onVoiceToggle && (
              <button
                ...
                disabled={disabled || optimizing || (!handsFreePhase && (micIsModeSwitch ? captureInFlight : transcribeInFlight))}
                aria-label={micLabel}
                title={micLabel}
              >
                {!handsFreePhase && !micIsModeSwitch && transcribeInFlight ? <Loader2 size={18} className="animate-spin" /> : voiceHoldMode ? <Keyboard size={18} /> : <Mic size={18} />}
              </button>
            )}
