...
const FILE_ACCEPT =
  ...
  "," +
  VIDEO_ACCEPT +
  ",.txt,.md,.json,.excalidraw,.har,.yaml,.yml,.xml,.csv,.log,.py,.js,.ts,.tsx,.jsx,.html,.css,.sh,.bash,.rb,.go,.rs,.java,.c,.cpp,.h,.hpp,.pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx,.odt,.ods,.odp,.rtf,.zip,.tar,.gz";
...
function nameClipboardImage(f: File, batchIndex: number): File {
  ...
  const generic =
    !f.name || f.name === `image.${ext}` || f.name === "image.png";
  if (!generic) return f;
  const d = new Date();
  ...
  return new File([f], `pasted-image-${stamp}${suffix}.${ext}`, {
    type: f.type,
  ...
  return (
    ...
    <div className="relative" ref={rootRef}>
      ...
      <div
        ...
        className="flex gap-2 px-4 py-2 border-t border-border bg-chrome/50 overflow-x-auto items-start"
        data-image-scope=""
      >
        {imgs.map((path, i) => {
          ...
          return (
            ...
            >
              ...
              <div className="relative">
                <span className="absolute -top-1.5 -left-1.5 w-5 h-5 rounded-full bg-accent text-accent-fg text-[10px] font-bold flex items-center justify-center z-10">
                  {i + 1}
                </span>
                <button
                  type="button"
                ...
                >
                  ...
                  <img
                    src={src}
                    alt={path}
                    className="w-16 h-16 rounded border border-border object-cover bg-bg-hover hover:opacity-80 transition-opacity"
                    data-lightbox-image=""
      ...
      {edges.left && (
        <div
          aria-hidden="true"
          data-testid="preview-strip-cue-left"
          className="pointer-events-none absolute left-0 top-px bottom-0 w-6 z-10 bg-gradient-to-r from-bg-elevated to-transparent"
        />
      )}
      {edges.right && (
        <div
          aria-hidden="true"
          data-testid="preview-strip-cue-right"
          className="pointer-events-none absolute right-0 top-px bottom-0 w-6 z-10 bg-gradient-to-l from-bg-elevated to-transparent"
        />
      )}
    </div>
  );
...
function ChatInput({
  ...
  const handleApprovalAction = useCallback(
    (decision: string, pattern?: string) => {
      ...
      if (
        ...
        api
          .resolveApproval(approvalId, toApiDecision(decision))
          .then(finish)
          .catch(fail);
      }
    },
  ...
  useLayoutEffect(() => {
    ...
    if (!pendingRef || pos == null) {
      ...
      if (el && voiceCaretRef && voiceCaretRef.current)
        voiceCaretRef.current = {
  ...
  const undoHistoryRef = useRef<UndoSnap[]>([
    {
      value,
    ...
    if (wasOptimizingRef.current && !optimizePending) {
      ...
      if (originating === slotId) {
        ...
        if (hist[ptr]?.value !== v) {
          ...
          hist.push({
            value: v,
            selStart: el?.selectionStart ?? v.length,
            selEnd: el?.selectionEnd ?? v.length,
            blocks: pasteBlocksRef.current,
          });
          if (hist.length > UNDO_MAX_HISTORY) hist.shift();
          undoPointerRef.current = hist.length - 1;
              ...
              {pendingSpawnApprovals.length > 1 && (
                <div className="px-3.5 pb-2.5 flex flex-col gap-1.5">
                  {pendingSpawnApprovals.map((a) => (
                    <div
                      key={a.id}
                      className="flex items-center gap-2 rounded-lg border border-border/60 bg-bg/40 px-2.5 py-1.5"
                    >
                      <code
                        className="text-[11px] font-mono text-muted/80 flex-1 min-w-0 truncate"
                        title={a.task || a.agent || a.id}
                      >
                        {a.task || a.agent || a.id}
                      </code>
                      {a.approving ? (
              ...
              <div className="flex items-center gap-1.5 px-3.5 py-2.5 select-none flex-wrap">
                {!showGhost && (
                  <>
                    ...
                    <span className="text-[13px] font-mono text-muted truncate flex-1 min-w-0">
                      {approvalLabel}
                    </span>
                  </>
                )}
                ...
                <div className="flex gap-1.5 flex-wrap items-center">
                  ...
                  {approvalIsReadOnly && approvalTrustGrantable && (
                    <button
                      disabled={approvalSubmitting}
                      className={approvalBtnClass}
                      onClick={() => handleApprovalAction("trust_reads")}
                    >
                      <BookOpen size={12} className="shrink-0" />
                      {i18nT("components.chatInput.trust_reads")}
                    </button>
                  )}
              ...
              >
                ...
                {pasteBlocks.length > 0 && (
                  <PasteHoverLayer
                    ...
                    blocks={pasteBlocks}
                    mirrorRef={mirrorRef}
                    onActivePanelChange={setPastePreviewPanelId}
                  />
                )}
              </div>
                ...
                <div className="flex items-center gap-1 shrink-0">
                  ...
                  {onVoiceToggle && (
                    <button
                      ...
                      onClick={
                        micIsModeSwitch ? toggleVoiceMode : onVoiceToggle
                      }
                      ...
                      onPointerDown={
                        micIsModeSwitch ? undefined : onVoicePrewarm
                      }
            ...
            {contextPct != null &&
              (() => {
                ...
                const readout = shelfCompact
                  ? composeContextReadout(contextPct, used, win, {
                      approx,
                      showPct,
                      showTokens: showTokens && !showPct,
                    })
                  : composeContextReadout(contextPct, used, win, {
                      approx,
                      showPct,
                ...
                return (
                  <div ref={ctxWrapRef} className="relative flex items-center">
                    ...
                    >
                      ...
                      {!!readout && (
                        <span
                          className="text-[11px] ml-1.5 tabular-nums whitespace-nowrap"
