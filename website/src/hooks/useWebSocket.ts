    ...
    ws.onmessage = (e) => {
      try {
        ...
        switch (type) {
          ...
          case 'subagent_spawn':
            dispatch(sseSubagentSpawn(data as { slot: string; id: string; task: string; agent: string; model?: string; controllable?: boolean }))
            break
          ...
          case 'subagent_recovering':
            ...
            break
          case 'subagent_done':
            dispatch(sseSubagentDone(data as { slot: string; id: string; elapsed: number; error?: string; stopped?: boolean; reported?: boolean; controllable?: boolean; outcome?: 'completed' | 'failed' | 'stopped' | 'reported'; task?: string; agent?: string; model?: string; result?: string }))
            break
          case 'app_reload':
          ...
          case 'subagent_snapshot':
            dispatch(sseSubagentSnapshot(data as { id: string; slot: string; task: string; agent: string; model?: string; streaming: string; last_tool: string; started: number; tool_count?: number; stalled?: boolean; controllable?: boolean }))
            break
