...
import {
  useDrawerSwipe,
  animateDrawer,
  registerDrawerTargets,
  takeOverDrawer,
  safeAreaLeft,
} from "../hooks/useDrawerSwipe";
...
import {
  ...
  isPlanAction,
} from "../hooks/usePlanActionMutation";
...
import {
  extractPromptFromToken,
  extractSlackContextFromToken,
} from "../utils/tokenPrompt";
...
import ChatDropOverlay, {
  useChatFileDrop,
} from "../components/ChatDropOverlay";
...
import SearchHighlightContext, {
  MessageSearchScope,
} from "../hooks/SearchHighlightContext";
...
import QueueStack, {
  SubagentDeliveryProgress,
  isSystemDelivery,
  isNonInteractiveQueued,
} from "../components/QueueStack";
...
} from "./chat/PeerChannelRequestCard";
...
const uniqueNotificationTs = (): string =>
  `${Date.now()}.${notificationTsSeq++}`;
...
export function ChatHeaderMenu({
  ...
  const { data: toolSearchOn = true } = useQuery<
    { agent?: { tool_search?: boolean } },
    Error,
    boolean
  >({
  ...
  const toolsByServer = useMemo(
    () =>
      Object.fromEntries(
        fullServers.map((s) => [
          s.name,
          { tools: s.tools, disabledTools: s.disabledTools },
        ]),
      ),
    [fullServers],
  );
  ...
  const loadedTools = useMemo(
    () => deriveLoadedMcpTools(sessionMessages),
    [sessionMessages],
  );
  ...
  const sessionReport = useAppSelector(
    (s) =>
      (activeSlot
        ? s.dashboard.slots?.find((sl) => sl.key === activeSlot)?.mcp_report
        : null) ?? null,
  );
  ...
  return (
    <DropdownMenu open={open} onOpenChange={setOpen}>
      <DropdownMenuTrigger asChild>
        <button
          className="px-0.5 py-1 rounded-md text-muted hover:text-text cursor-pointer bg-transparent border-none transition-all"
          aria-label={i18nT("pages.chatPage.session_options")}
        >
          <ChevronDown size={14} />
        </button>
      </DropdownMenuTrigger>
              ...
              <DropdownMenuSub key="mcp" onOpenChange={setMcpOpen}>
                <DropdownMenuSubTrigger>
                  ...
                  <span className="flex-1">
                    {i18nT("pages.chatPage.mcp_servers")}
                  </span>
...
export function turnLeadKey(
  ...
  return lead
    ? `grp-${msgIdentityKey(lead, msgKey)}`
    : `grp-idx-${it.startIdx}`;
}
  ...
  return (
    <span className="block mb-1">
      <button
        ...
        aria-label={
          expanded
            ? i18nT("pages.chatPage.collapse_knowledge_context")
            : i18nT("pages.chatPage.expand_knowledge_context")
        }
      >
        <BookOpen size={12} className="shrink-0" />{" "}
        {i18nT("pages.chatPage.knowledge_item", { count: knowledge.items })} ·{" "}
        {fmtNumber(knowledge.tokens)} {i18nT("pages.chatPage.tokens")}
      </button>
        ...
        <div className="mt-1 max-h-[300px] overflow-auto rounded border border-border bg-bg-elevated p-2 text-[11px]">
          {knowledge.content.map((item, i) => (
            <div key={i} className="mb-2 last:mb-0">
              ...
              <pre
                className="mt-0.5 whitespace-pre-wrap text-muted font-mono leading-[1.4]"
...
function renderUserContentInner(
  content: string,
  meta: Record<string, unknown> | undefined,
  onFileOpen: (path: string) => void,
  onFolderOpen?: (path: string) => void,
  ...
  const knowledge = meta?.knowledge as
    | {
        items: number;
        tokens: number;
        titles: string[];
  ...
  const { display: dirResolved, dirMentionMap } = resolveDirSegment(
    content,
    parseDirs(content, meta),
  );
  content = dirResolved;
  ...
  const unreferenced = orderedFiles.length
    ? findUnreferencedAttachments(text, orderedFiles)
    : [];
...
function DirChip({
  ...
  const body = (
    <>
      <Folder size={11} aria-hidden="true" className="shrink-0 lucide-inline" />
      @{label}
    </>
  );
  if (!onOpen) {
    return (
      <span
        className="inline-flex items-center gap-1 px-1.5 py-0.5 mx-0.5 rounded border border-accent/25 bg-accent/10 text-accent text-[12px] font-mono"
        title={fullPath}
      >
        {body}
      </span>
...
function renderInlineSegment(
  ...
  const dirKeys = dirMap
    ? [...dirMap.keys()].filter((k) => tokenPresent(content, k)).slice(0, 20)
    : [];
  ...
  const { display, mentionMap, cardPaths, labels } = resolveFileSegment(
    content,
    parsedFiles,
  );
  ...
  const parts = tokPattern
    ...
    : [display];
  const chipCls =
    "inline-flex items-center px-1.5 py-0.5 mx-0.5 rounded bg-accent/15 text-accent text-[12px] font-mono cursor-pointer hover:bg-accent/25 transition-colors";
  return (
    <span key={keyBase} style={{ whiteSpace: "pre-wrap" }}>
      {parts.map((part, i) => {
      ...
      {cardPaths.map((p, i) => (
        <Clickable
          ...
          aria-label={i18nT("pages.chatPage.open_file", { path: p })}
        >
          @{labels.get(p) || p}
        </Clickable>
      ))}
...
function renderFileSegment(
  ...
  const allCardPaths = [
    ...cardPaths,
    ...findUnreferencedAttachments(display, parsedFiles).filter(
      (p) => !carded.has(p),
    ),
  ];
  ...
  const cards = allCardPaths.length ? (
    <div
      key={`${keyBase}-cards`}
      className="flex flex-col gap-1.5 mt-1 first:mt-0"
    >
      ...
      {parts.map((part, i) => {
        ...
        if (fullPath) {
          return (
            <Clickable
              key={`${keyBase}-f${i}`}
              className="inline-flex items-center px-1.5 py-0.5 mx-0.5 rounded bg-accent/15 text-accent text-[12px] font-mono cursor-pointer hover:bg-accent/25 transition-colors"
              title={fullPath}
              onClick={() => onFileOpen(fullPath)}
              aria-label={i18nT("pages.chatPage.open_file", { path: fullPath })}
            >
 ...
 */
function unresumableNoticeMessage(r: {
  key: string;
  title: string;
  surface: string;
  ...
  if (!surface) {
    return i18nT("pages.chatPage.this_session_is_not_a_chat_session", {
      title,
    });
  }
  return i18nT("pages.chatPage.this_session_cannot_be_opened_in_chat", {
    title,
    surface,
  });
}
...
const jumpUnavailableNotice = (origin: PendingJumpOrigin): string =>
  origin === "earlier"
    ...
    : origin === "link"
      ? i18nT("pages.chat.deepLink.message_unavailable")
...
export default function ChatPage({
  ...
  const appToolCallIds = useAppSelector(
    (s) => {
  ...
  const { data: appPanelCfg, isError: appPanelCfgError } = useQuery<{
  ...
  const kiroCrewVersion =
    useAppSelector((s) => s.dashboard.status?.version) || "";
  ...
  const completedTurnCount = useMemo(
    () => countCompletedTurns(messages),
    [messages],
  );
  ...
  const pendingFollowup = useAppSelector((s) =>
    s.chat.activeSlot ? s.chat.followups?.[s.chat.activeSlot] : undefined,
  );
  const folderSuggestion = useAppSelector((s) =>
    s.chat.activeSlot
      ? s.chat.folderSuggestions?.[s.chat.activeSlot]
      : undefined,
  );
  const followupTsBySlot =
    useAppSelector((s) => s.chat.followups) ?? EMPTY_FOLLOWUPS;
  ...
  const tipSuppressed =
    useAppSelector(
      (s) =>
        ...
        // forever.
        Object.values(s.chat.workflowRuns ?? {}).some(
          (r) =>
            runBelongsToSlot(r.sessionKey, s.chat.activeSlot) &&
            (r.status === "running" ||
              r.status === "finished" ||
              r.status === "failed" ||
              r.status === "cancelled"),
    ...
    knowledgeFetch.loading ||
    knowledgeFetch.results.length > 0;
  ...
  const navigateToEmbeddedSlot = useCallback(
    (key: string) => navigate(`/embed/chat/${key}`),
    [navigate],
  );
  ...
  const tipTemporary = useAppSelector(
    (s) =>
      s.dashboard.slots.find((sl) => sl.key === s.chat.activeSlot)
        ?.memory_mode === "temporary",
  );
  const tipBlocked = tipTemporary || splitMode || embedMode === "sessions";
  const { tip: activeTip, dismiss: dismissTip } = useTipTrigger(
    !!slotRunning,
    tipSuppressed,
    activeSlot,
    tipBlocked,
  ...
  const contextPct = useAppSelector(
    (s) => s.chat.slotContextPct[s.chat.activeSlot ?? ""],
  );
  const contextTokens = useAppSelector(
    (s) => s.chat.slotContextTokens?.[s.chat.activeSlot ?? ""],
  );
  ...
  const cursorIsForActiveSlot = useAppSelector(
    (s) => s.chat.slotCursorKey === s.chat.activeSlot,
  );
  ...
  const activeViewIsBoundedPage = useAppSelector((s) =>
    activeSlot ? s.chat.slotPaneBounded?.[activeSlot] !== undefined : false,
  );
  ...
  if (sessionRefDrafts.current === null)
    sessionRefDrafts.current = loadSessionRefDrafts();
  const saveDraftsTimer = useRef<ReturnType<typeof setTimeout> | null>(null);
  const saveDrafts = useCallback(() => {
    persistDrafts(drafts.current);
    persistFileDrafts(fileDrafts.current);
    persistPasteDrafts(pasteDrafts.current);
    persistSessionRefDrafts(sessionRefDrafts.current);
  }, []);
  const saveDraftsDebounced = useCallback(() => {
    if (saveDraftsTimer.current) clearTimeout(saveDraftsTimer.current);
    saveDraftsTimer.current = setTimeout(() => {
      saveDraftsTimer.current = null;
      saveDrafts();
    }, DRAFT_SAVE_DEBOUNCE_MS);
  }, [saveDrafts]);
  const flushDrafts = useCallback(() => {
    if (saveDraftsTimer.current) {
      clearTimeout(saveDraftsTimer.current);
      saveDraftsTimer.current = null;
    }
    saveDrafts();
  }, [saveDrafts]);
  ...
  const [input, setInput] = useState(() =>
    activeSlot ? (drafts.current[activeSlot] ?? "") : "",
  );
    ...
    () =>
      historyQuery && history.length
        ? history
            .filter(
              (s) =>
                (s.title || "").toLowerCase().includes(historyQuery) ||
                s.key.toLowerCase().includes(historyQuery),
            )
            .slice(0, 5)
        : [],
  ...
  const isWelcomeState =
    ...
    !slotLoading &&
    !sendingRef.current &&
    !knowledgeFetch.results.length &&
    !knowledgeFetch.loading &&
    !knowledgeFetch.pendingKnowledge &&
    !pendingQuestion;
  const showHistorySuggestions =
    isWelcomeState && historySuggestions.length > 0 && !historyDismissed;
  useEffect(() => {
    if (!showHistorySuggestions) return;
    const onKey = (e: KeyboardEvent) => {
      if (e.key === "Escape") setHistoryDismissed(true);
    };
  ...
  const activeSlotProject =
    slots.find((s) => s.key === activeSlot)?.project || undefined;
  ...
  const {
    open: agentDropdown,
    setOpen: setAgentDropdown,
    filter: agentFilter,
    setFilter: setAgentFilter,
    dropdownRef: agentDropdownRef,
    inputRef: agentInputRef,
    filtered: filteredAgentsByName,
  } = useFilteredDropdown(installedAgents);
  ...
  const setPendingAgent = useCallback((v: string) => {
  ...
  const setPendingModel = useCallback((v: string) => {
    pendingModelRef.current = v;
    _setPendingModel(v);
  }, []);
  const pendingProjectRef = useRef("");
  const setPendingProject = useCallback((v: string) => {
    pendingProjectRef.current = v;
  }, []);
  const modelPickerAgentName =
    slots.find((s) => s.key === activeSlot)?.agent ||
    pendingAgent ||
    defaultAgent;
  const modelPickerAgent = installedAgents.find(
    (agent) => agent.name === modelPickerAgentName,
  );
  ...
  } = useFilteredDropdown(availableModels);
  ...
  const steerMutation = useMutation({
    mutationFn: ({
      ...
      sendId?: string;
      slot: string;
    }) => api.steerChat(text, slot, sendId),
  ...
  useEffect(() => {
    ...
    const handler = (e: MouseEvent) => {
      if (reasoningEffortDropdownRef.current?.contains(e.target as Node))
        return;
  ...
  useEffect(() => {
    ...
    fetch(`/api/autonudge/slot/${encodeURIComponent(activeSlot)}`)
      .then((r) => r.json())
      .then((d) => {
        if (!cancelled) setAutoNudgeLoop(d.loop || null);
      })
      .catch(() => {});
    const onEvent = (e: Event) => {
      ...
      setAutoNudgeLoop(
        detail.event === "removed" ? null : (detail.loop ?? null),
      );
    };
    window.addEventListener("autonudge_state", onEvent);
    return () => {
      cancelled = true;
      window.removeEventListener("autonudge_state", onEvent);
    };
  }, [activeSlot]);
  const { scrollerRef, scrollToDisplayIndex } = useScrollManager();
    ...
    const restageFailure = (error: unknown) => {
      ...
      dispatch(
        addNotification({
          ...
          body: i18nT(
            "pages.chatPage.could_not_start_session_message_restored",
            {
    ...
    try {
      ...
      try {
        const slot = await dispatch(
          createSlot({ mode: errorHandoffModeRef.current, activate: false }),
        ).unwrap();
      ...
      try {
        ...
        await dispatch(
          switchSlot({ key: slotKey, keepTargetOnMissing: true }),
        ).unwrap();
      } catch {
      ...
      for (let i = 0; i < 300 && ownsLifecycle(); i++) {
        ...
        if (Object.prototype.hasOwnProperty.call(drafts.current, slotKey))
          break;
  ...
  useEffect(() => {
    if (pendingInput) {
      ...
      if (shouldAutoSend) {
        ...
        if (activeSlot) {
          setDraft(drafts.current, activeSlot, pendingInput);
          saveDraftsDebounced();
        }
      ...
      try {
        ...
        if (!slotKey) {
          ...
          if (slotKey && channel && threadTs) {
            try {
              await api.slackLink(slotKey, channel, threadTs);
            } catch {
              /* non-fatal */
            }
          }
  ...
  useEffect(() => {
    ...
    if (s) {
      setDraft(drafts.current, s, input);
      saveDraftsDebounced();
    }
  }, [input, saveDraftsDebounced]);
  ...
  useEffect(() => {
    ...
    for (const [k, v] of Object.entries(stored)) {
      if (!(k in drafts.current)) drafts.current[k] = v;
    }
    const storedFiles = loadFileDrafts();
    for (const [k, v] of Object.entries(storedFiles)) {
      if (!(k in fileDrafts.current)) fileDrafts.current[k] = v;
    }
    const storedPastes = loadPasteDrafts();
    for (const [k, v] of Object.entries(storedPastes)) {
      if (!(k in pasteDrafts.current)) pasteDrafts.current[k] = v;
    }
    const storedSessionRefs = loadSessionRefDrafts();
    for (const [k, v] of Object.entries(storedSessionRefs)) {
      if (!(k in sessionRefDrafts.current)) sessionRefDrafts.current[k] = v;
    }
    if (prevSlot.current)
    ...
    if (raw) {
      try {
        ...
        if (Date.now() - (ts ?? 0) > 30_000) {
          sessionStorage.removeItem(PREFILL_STORAGE_KEY);
          setInput(draftFallback);
      ...
      prevSlotVal === activeSlot &&
      !!activeSlot &&
      consumedPrefillRef.current?.startsWith(`${activeSlot}:`)
    ) {
    ...
    setPendingFiles(
      activeSlot ? (fileDrafts.current[activeSlot] ?? []).slice() : [],
    );
  ...
  const pendingDirs = useMemo(
    () => parseDirTokens(input).map((t) => t.rel),
    [input],
  );
  ...
  useEffect(() => {
    composerSlotRef.current = activeSlot;
  }, [activeSlot]);
  ...
  const spliceDictation = useCallback(
    (base: string, text: string): { value: string; caret: number } => {
      ...
      if (!text)
        return {
          value: base,
          caret: caret ? Math.min(caret.start, base.length) : base.length,
        };
      if (!caret) {
        const value = base
          ? base.endsWith(" ")
      ...
      return {
        value: before + insert + trail + after,
  ...
  const applyVoiceText = useCallback(
    (text: string, sessionId: string | null, origin: TranscriptOrigin) => {
      ...
      const append = (base: string) =>
        base ? (base.endsWith(" ") ? base + text : base + " " + text) : text;
  ...
  const { data: sourceHostCfg } = useQuery<{
    gitlab_hosts?: string[];
    jira_hosts?: string[];
  }>({
    queryKey: ["dashboardConfig"],
  ...
  const [sourceSelections, setSourceSelections] =
    useState(loadSourceSelections);
  const selectedSourceUrl = sourceSelection(
    sourceSelections,
    activeSlot,
    "change",
  );
  const selectedIssueUrl = sourceSelection(
    sourceSelections,
    activeSlot,
    "issue",
  );
  ...
  const [revealedSources, setRevealedSources] =
    useState<RevealedSources>(loadRevealedSources);
  ...
  const panelSources = useMemo(
    () =>
      revealedChange &&
      !sourceLinks.some((link) => link.url === revealedChange.url)
  ...
  const panelIssues = useMemo(
    () =>
      revealedIssue &&
      !issueLinks.some((link) => link.url === revealedIssue.url)
  ...
  const unpersistedSelectionsRef = useRef<
    Record<string, Partial<Record<SourceLinkKind, boolean>>>
  >({});
  ...
  // restore.
  const provisionalFallbackRef = useRef<
    Record<string, Partial<Record<SourceLinkKind, number>>>
  >({});
  const selectSource = useCallback(
    (kind: SourceLinkKind, url: string, forSlot?: string) => {
      ...
      setSourceSelections((previous) =>
        withSourceSelection(previous, slot, kind, url),
      );
  ...
  const selectSourceUrl = useCallback(
    (url: string) => selectSource("change", url),
    [selectSource],
  );
  const selectIssueUrl = useCallback(
    (url: string) => selectSource("issue", url),
    [selectSource],
  );
  ...
  const reconcileSelection = useCallback(
    (kind: SourceLinkKind, url: string, seen = 0) => {
  ...
  const restoreFromStorage = useCallback(
    (kind: SourceLinkKind, links: readonly { url: string }[]): boolean => {
      const slot = activeSlotRef.current;
      ...
      if (stored && links.some((link) => link.url === stored)) {
        ...
        setSourceSelections((previous) =>
          withSourceSelection(previous, slot, kind, stored),
        );
        return true;
      }
      ...
      provisionalFallbackRef.current = {
        ...provisionalFallbackRef.current,
        [slot]: {
          ...provisionalFallbackRef.current[slot],
          [kind]: links.length,
        },
      };
      return false;
  ...
  useEffect(() => {
    const newChanges = recordNewPullRequestLinks(
      seenSourceUrls,
      activeSlot,
      sourceLinks,
    );
    const newIssues = recordNewPullRequestLinks(
      seenSourceUrls,
      activeSlot,
      issueLinks,
    );
  ...
  useEffect(() => {
    ...
    if (panelSources.length === 0) {
      ...
      if (messages.length && selectedSourceUrl)
  ...
  const handleFileOpen = useCallback(
    async (
      ...
      try {
        window.dispatchEvent(
          new CustomEvent("kirocrew-file-open", { detail: { path: filePath } }),
        );
      } catch {
        /* ignore */
      }
      if (
        (window as unknown as { __kirocrewPluginHandlesFiles?: boolean })
          .__kirocrewPluginHandlesFiles
      )
        return;
      try {
          ...
          queryClient.fetchQuery({
            ...
            queryFn: async () => {
              ...
              const text = res.ok
                ...
                : res.status === 404
                  ? i18nT(
                      "pages.chatPage.file_not_found_on_disk_it_may_have_been_moved_or",
                    )
        ...
        tabsCtl.openFile(
          filePath,
          i18nT("pages.chatPage.error_reading_file"),
          activeSlotRef.current ?? null,
          optsForReplace(opts),
        );
        dispatch(openActivityPanel());
  ...
  const handleSubagentPanelOpen = useCallback(
    (parsed: ParsedSubagentCompletion) => {
  ...
  const handleArtifactOpen = useCallback(
    async (slug: string) => {
      ...
      if (slot) {
        api
          .recordArtifactReference(slug, slot)
          .then(() => {
      ...
      try {
        ...
        /* fall through — the panel's own query renders the error state */
      }
  ...
  const handleOpenDiff = useCallback(
    (filePath: string, modified: string, original: string) => {
      ...
      try {
        window.dispatchEvent(
          new CustomEvent("kirocrew-file-open", {
      ...
      if (!original || !original.trim() || original === modified) {
  ...
  const handleFork = useCallback(
    async (visibleIndex: number, messageId?: string) => {
      if (!activeSlot) return;
      try {
        ...
        if (result.ok) {
          ...
          alert(
            i18nT("pages.chatPage.fork_failed_error", {
              error: result.error || i18nT("pages.chatPage.unknown_error"),
            }),
          );
        }
      } catch (e) {
        alert(
          i18nT("pages.chatPage.fork_failed_error", {
            error: errMessage(e) || i18nT("pages.chatPage.unknown_error"),
          }),
        );
      }
    },
  ...
  const handlePlanFromHere = useCallback(
    async (visibleIndex: number, messageId?: string) => {
      if (!activeSlot) return;
      try {
        const result = await dispatch(
        ...
        if (result.ok) {
          ...
          alert(
            i18nT("pages.chatPage.plan_from_here_failed_error", {
              error: result.error || i18nT("pages.chatPage.unknown_error"),
            }),
          );
        }
  ...
  const handleFileSave = useCallback(
    async (filePath: string, content: string) => {
      ...
      if (getInlineDraft(requestSlot, filePath) === content)
        clearInlineDraft(requestSlot, filePath);
    },
    [tabsCtl],
  );
  ...
  const uploadFiles = useCallback(
    async (files: File[], targetSlot?: string | null) => {
      ...
      const requestSlot =
        targetSlot !== undefined ? targetSlot : activeSlotRef.current;
      setUploadError("");
      if (files.length > 20) {
      ...
      const big = files.find(
        (f) => !VIDEO_EXT.test(f.name) && f.size > 50 * 1024 * 1024,
      );
      if (big) {
        setUploadError(
          i18nT("pages.chatPage.file_too_large", { name: big.name }),
        );
        return;
      }
      setUploading(true);
      try {
        ...
        if (res.error) {
          ...
          if (landing.target === "pending") {
            ...
            setFileDraft(fileDrafts.current, landing.slot, [
              ...cur,
              ...res.paths,
            ]);
            saveDrafts();
        ...
        if (
          !res.error &&
          res.resizedByPath &&
          Object.keys(res.resizedByPath).length
        ) {
        ...
        setUploadError(
          i18nT(
            "pages.chatPage.upload_failed_check_file_type_and_size_max_50_mb",
          ),
        );
      }
      setUploading(false);
  ...
  const handleOptimizeResult = useCallback(
    (slot: string | null, optimized: string) => {
      if (!slot) return;
  ...
  const handleDrop = useCallback(
    (dataTransfer: DataTransfer) => {
      ...
      if (dirPaths.length) {
        ...
        const rels = dirPaths.map((p) =>
          makeRelative(p, currentProjectRef.current || ""),
        );
        const spliced = spliceDirTokens(
          inputRef.current,
          voiceCaretRef.current?.start ?? null,
          rels,
        );
        if (spliced.changed) {
  ...
  const handleSurveyLayoutChange = useCallback(
    () => setSurveyLayoutTick((t) => t + 1),
    [],
  );
  useEffect(() => {
  ...
  const composerBandRef = useCallback(
  ...
  const navToDisplayIndex = useCallback(
    (
      ...
      const behavior: ScrollBehavior = jumpedFar
        ? "auto"
      ...
      const detachUserScroll = attachUserScrollIntent(
        scrollEl ?? undefined,
        onUserScroll,
      );
      navPollCancelRef.current = pollRowSettled({
        ...
        step: glideOnceStep((b) => {
          scrollToDisplayIndex(idx, { ...opts, behavior: b });
        }, behavior),
  ...
  const updatePinnedPrompt = useCallback(() => {
    ...
    const foldY =
      pinFoldRef.current?.getBoundingClientRect().top ??
    ...
    for (const item of items) {
      ...
      if (htmlItem.getBoundingClientRect().bottom > handoffY) {
        handoffIdx = parseInt(
          htmlItem.getAttribute("data-display-index") || "0",
          10,
        );
        break;
      }
    ...
    if (!pinEnabledRef.current || handoffIdx < 0) {
      setPinned(null);
      return;
    }
    ...
    if (!pinItem || pinItem.kind !== "single") {
      setPinned(null);
      return;
    }
    ...
    const measured = minimized
      ? 0
      : (pinCardRef.current?.getBoundingClientRect().height ?? 0);
    ...
    if (!minimized && push >= pinPushTravel(bannerH)) {
      setPinned(null);
      return;
    }
    ...
    const nudge =
      pinItem.msg.role === "nudge" ? parseNudgeMessage(pinItem.msg) : null;
  ...
  const scrollToPinnedPrompt = useCallback(
    (target: number) => {
      ...
      const detach = attachUserScrollIntent(
      ...
      const tick = () => {
        ...
        if ((h != null && stable >= 2) || frames >= 30) {
          ...
          detach();
          detach2 = attachUserScrollIntent(
            scrollerRef.current ?? undefined,
            () => {
              cancelled = true;
            },
          ...
          const glide = () => {
            ...
            const goal = Math.max(
              0,
              Math.min(sc.scrollHeight - sc.clientHeight, liveTarget),
            );
            const t = reduced
              ? 1
              : Math.min(1, (performance.now() - t0) / GLIDE_MS);
  ...
  const slotStorageKeyRef = useRef(slotStorageKey);
  slotStorageKeyRef.current = slotStorageKey;
  useEffect(() => {
  ...
  useEffect(
    () => () => {
      if (
        activeSlotRef.current &&
        filteredSlotsRef.current.find((s) => s.key === activeSlotRef.current)
      )
        safeSetItem(slotStorageKeyRef.current, activeSlotRef.current);
    },
    [],
  );
   ...
   */
  const openSlotInNewTab = useCallback(
    (key: string, opts?: { background?: boolean }) => {
  ...
  const closeSessionTab = useCallback(
    (key: string) => {
      ...
      if (next && next !== activeSlotRef.current && connectedRef.current)
        dispatch(switchSlot(next));
  ...
  const initialSidRef = useRef(
    noUrlSync ? null : searchParams.get("sid") || searchParams.get("slot"),
  );
  ...
  const deepLinkPendingRef = useRef(
    !!initialSidRef.current && initialSidRef.current !== activeSlot,
  );
  ...
  const newSlotMutation = useMutation({
    ...
    onSuccess: (slot) => {
      ...
      navigate(
        embedMode
          ? `/embed/chat/${slot.key}`
          : `/chat?sid=${encodeURIComponent(slot.key)}`,
        { replace: true },
      );
    },
    onError: () => {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      if (initialSidRef.current) {
        ...
        popInFlightRef.current = false;
        setSidError(
          i18nT("pages.chatPage.session_not_found", { name: urlSlot }),
        );
  ...
  useEffect(() => {
    ...
    const slug =
      slot?.title && slot.title !== slot.key ? toSlug(slot.title) : "";
    ...
    navigate(`${basePath}${slug ? "/" + slug : ""}?${next}`, {
      replace: shouldReplaceSessionUrl({ isSessionSwitch, isMobile }),
    });
  ...
  useEffect(() => {
    if (
      ...
      filteredSlotsRef.current.find((s) => s.key === activeSlot)
    )
      dispatch(switchSlot(activeSlot));
  }, []); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  useEffect(() => {
    if (
      activeSlot &&
      slots.length > 0 &&
      !filteredSlots.find((s) => s.key === activeSlot)
    ) {
  ...
  useEffect(() => {
    ...
    if (filteredSlots.length > 0) {
      ...
      const target =
        saved && filteredSlots.find((s) => s.key === saved)
          ? saved
          : filteredSlots[0].key;
  ...
  const planActionMutation = usePlanActionMutation(
    activeSlot,
    followUpSourceKey,
  );
  ...
  const [followUpPicked, setFollowUpPicked] = useState<Set<string>>(
    () => new Set(),
  );
  ...
  const {
    isPoppedOut: isSlotPoppedOut,
    open: openActivePopout,
    focus: focusActivePopout,
    returnSelfToMain,
  } = useChatPopouts();
  ...
  const handleResumeSession = useCallback(
    async (key: string, title: string) => {
      try {
        ...
        /* resume failed — keep current slot */
      }
    },
  ...
  const send = useCallback(
    async (optionText?: string, targetSlot?: string, steerNow?: boolean) => {
      ...
      const widgetOrigin =
        !!widgetPrefillRef.current && raw.includes(widgetPrefillRef.current);
      widgetPrefillRef.current = null;
      if (
        !raw &&
        !pendingFilesRef.current.length &&
        !pendingSessionsRef.current.length
      )
        return;
      ...
      const cardAtSend = captureStatelessCard(
      ...
      const askAtSend = capturePendingAskId(
        store.getState().chat.pendingQuestions,
        entrySendSlot,
      );
      ...
      if (isInterceptedSlashCommand(raw)) {
        ...
        const slashTxt = slashPastes.length
          ...
          : raw;
        const slashResult = await interceptSlashCommand(
          slashTxt,
          uiSlot,
          dispatch,
        );
      ...
      const sentSessionRefs = optionText
        ? []
        : pendingSessionsRef.current.slice();
      const {
        txt: typedTxt,
      ...
      const displayTxt = appendSessionRefLinks(
        typedDisplayTxt,
        sentSessionRefs,
      );
      ...
      let llmTxt = activePastes.length
        ? expandPasteTokens(txt, activePastes)
        : txt;
      // Prepend knowledge context if pending
      let knowledgeBlock:
        import("./chat/useKnowledgeFetch").KnowledgeBlock | null = null;
      ...
      if (bubblePastes.length)
        saveStoredPaste(llmTxt, displayTxt, bubblePastes, filePaths);
      ...
      if (!slot || forceNew) {
        ...
        try {
          ...
          if (onScreen && uiSlot) {
            ...
            if (knowledgeBlock) {
              const newer =
                knowledgeFetchRef.current.pendingKnowledge?.items ?? [];
            ...
            if (retryNow) {
              ...
              dispatch(
                addNotification({
                  ...
                  title: i18nT("pages.chatPage.could_not_start_a_new_session"),
                  body: i18nT(
                    "pages.chatPage.message_queued_until_session_ready",
                    { error: createFailReason(e) },
                  ),
                }),
              );
        ...
        if (pendingProjectRef.current) {
          await api
            .chatSlotProject(result.key, pendingProjectRef.current)
            .catch((e) => {
      ...
      try {
        ...
        void resolveAskAfterSend(
          body,
          slot === entrySendSlot ? askAtSend : null,
          dispatch,
        );
      } catch (e: unknown) {
        clearTimeout(timeout);
  ...
  const toApiDecision = useCallback(
  ...
  const dismissApproval = useCallback(
    (aid: string, decision?: string) => {
      ...
      const n = store
        .getState()
        .notifications.items.find((x) => x.approval_id === aid);
  ...
  const tabsCtlRef = useRef(tabsCtl);
  tabsCtlRef.current = tabsCtl;
  ...
  const revealAppInPanel = useCallback(
    (toolCallId: string) => {
      search.close();
      dispatch(openActivityPanel());
      tabsCtlRef.current?.openApp(
  ...
  const handleAddToContext = useCallback(
    (absPath: string, kind: "file" | "dir") => {
      ...
      const rel = makeRelative(
        absPath,
        normalizeWindowsPath(currentProjectRef.current || ""),
      );
      if (kind === "dir") {
        ...
        const projectIsWindowsShaped =
          normalizeWindowsPath(project) !== project;
        const dup =
          projectIsWindowsShaped &&
          parseDirTokens(inputRef.current).some(
  ...
  const followupTsRef = useRef<
    Record<string, { items: FollowupItem[]; ts: number }>
  >({});
  followupTsRef.current = followupTsBySlot;
  const followupAddToSession = useCallback(
    (item: FollowupItem) => {
      if (!activeSlot) return;
  ...
  const folderSuggestionAccept = useCallback(() => {
    ...
    dispatch(
      clearFolderSuggestion({ slot: activeSlot, ts: folderSuggestion.ts }),
    );
  ...
  const followupStartInWorktree = useCallback(
    async (item: FollowupItem) => {
      const repo = currentSlot?.project;
      if (!repo)
        throw new Error(
          i18nT(
            "pages.chatPage.this_session_has_no_project_directory_to_branch",
          ),
        );
      ...
      const originTs = originSlot
        ? followupTsRef.current[originSlot]?.ts
        : undefined;
      ...
      if (!path)
        throw new Error(
          res?.error ||
            i18nT("pages.chatPage.worktree_creation_returned_no_path"),
        );
      let slotKey = "";
      try {
        ...
        const slot = await dispatch(
          createSlot({ mode, project: path, activate: false }),
        ).unwrap();
        slotKey = slot?.key || "";
      } catch {
  ...
  useEffect(() => {
    ...
    try {
      existing =
        localStorage.getItem(`mc-webpreview-url:${slot}`) ||
    ...
    if (feed.open) {
      ...
      let applied = "";
      try {
        applied = localStorage.getItem(`mc-webpreview-applied:${slot}`) || "";
      } catch {
        /* ignore */
      }
      if (applied === norm || appliedPreviewMemRef.current[slot] === norm)
        return;
  ...
  const browseOpenedRef = useRef<{ key: string | null; ts: number }>({
    key: null,
    ts: 0,
  });
  useEffect(() => {
  ...
  useEffect(() => {
    const api = (
      window as unknown as {
        browserAPI?: {
          trackSession?: (id: string, tracked: boolean) => Promise<unknown>;
        };
      }
    ).browserAPI;
  ...
  useEffect(() => {
    const api = (
      window as unknown as {
        browserAPI?: {
          onAgentOpened?: (cb: (p: { panelId?: string }) => void) => () => void;
        };
      }
    ).browserAPI;
  ...
  useEffect(() => {
    const handler = (e: Event) => {
      ...
      const emit = (ok: boolean) => {
        ...
        settled = true;
        window.dispatchEvent(
          new CustomEvent("mc:run-in-terminal-result", {
            detail: { reqId, ok },
          }),
        );
      };
      ...
      const unsub = onTerminalReady(sessionId, () => {
        emit(sendToTerminalSession(sessionId, code));
      });
  ...
  const coldFileTabs = useMemo(
    () =>
      tabsCtl.tabs.filter(
        (t) => t.kind === "file" && t.path && t.content === undefined,
      ),
    [tabsCtl.tabs],
  );
  ...
  const title =
    currentSlot?.title && currentSlot.title !== currentSlot.key
      ? currentSlot.title
      : activeSlot || "";
  const displayMode =
    approvalMode === "yolo"
      ...
      : currentSlot?.trust
        ? "trust"
        : currentSlot?.trust_reads
          ? "trust_reads"
          : "normal";
  // Resolve model for existing slots that don't have one stored
  const _slotAgentName =
    currentSlot && !currentSlot.model
      ? currentSlot.agent || defaultAgent || "default"
      : "";
  ...
  const _modelPinAgent =
    currentSlot?.agent || pendingAgent || defaultAgent || "default";
  ...
  const _modelPinPinned =
    !!_modelPinCfg?.model &&
    _modelPinCfg.model === _modelPinActive &&
  ...
  const [sidebarPinned, setSidebarPinned] = useState(
    () => localStorage.getItem("mc-sidebar-pinned") !== "false",
  );
  ...
  const sidePanelDockAnim = useMemo(
    () => sidePanelDockMotion(sidePanelDock),
    [sidePanelDock],
  );
  ...
  const editingTitle =
    editingTitleSlot !== null && editingTitleSlot === activeSlot;
  ...
  const enterSplit = useCallback((anchor: string | null) => {
    setSplitAnchor(anchor);
    setSplitMode(true);
  }, []);
  ...
  const splitAnchorForActive = useMemo(() => {
    ...
    return sessionSlots(loadLayout(anchor)).filter((k) => liveKeys.has(k))
      .length >= 2
      ? anchor
      : null;
  ...
  const activeIsSplitAnchor =
    splitAnchorForActive !== null && splitAnchorForActive === activeSlot;
  ...
  const [generatingTitleSlots, setGeneratingTitleSlots] = useState<Set<string>>(
    new Set(),
  );
  ...
  const lastTextIdx = useMemo(() => {
    for (let i = messages.length - 1; i >= 0; i--) {
      ...
      if (
        ...
        !isHiddenInvisibleAssistantRow(messages[i])
      )
        return i;
    }
    return -1;
  ...
  const showRefusedPress = useCallback(
    (action: RefusedPressAction, e: unknown) => {
      setRefusedPress({
        action,
  ...
  useEffect(() => {
    if (slotRunning) setRefusedPress(null);
  }, [slotRunning]);
    ...
    (text: string) => {
      ...
      const trySeed = (attempt = 0) => {
        ...
        if (mounted || attempt >= 20) {
          window.dispatchEvent(
            new CustomEvent("side-seed", { detail: { text } }),
          );
        } else {
  ...
  const searchCtxValue = useMemo(
    ...
    [
      ...
      search.currentMessageIdx,
      search.currentOccurrenceIdx,
    ],
  );

  const renderUserContentCb = useCallback(
    (c: string, mt: Record<string, unknown> | undefined) =>
      renderUserContent(
        c,
        mt,
        handleFileOpen,
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("toggle-pin-chat-sidebar", togglePin);
  }, []);
  ...
  if (heatMessagesRef.current !== messages) {
    heatMessagesRef.current = messages;
    heatTickRef.current++;
  }
  ...
  useEffect(() => {
    if (!activityTabBridged.current) {
      activityTabBridged.current = true;
      return;
    }
    if (activityOpen)
      tabsCtl.openView(
        activityTab === ("nav" as string) ? "files" : activityTab,
      );
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [activityTabRequest]);
      ...
      api
        ...
        .then((result: { mode?: string; url?: string }) => {
          if (result.mode === "stream" && result.url) {
            window.dispatchEvent(
              new CustomEvent("voice-play-url", { detail: result.url }),
            );
            return;
          }
  ...
  const handleApplyPlan = useCallback(
    async (steps: PlanStepInput[]) => {
      try {
        const r = await api.planFromChat(steps, planTaskId);
        if (r.ok) {
          navigate("/projects?applied=" + (r.task_id || planTaskId));
          return true;
        }
      } catch {
        /* API error */
  ...
  const bubbleProbeCounts = useCallback(
    () => ({
      store: messagesLenRef.current,
      display: displayItemsRef.current.length,
    }),
    [],
  );
  ...
  useEffect(() => {
    updatePinnedPrompt();
  }, [displayItems, updatePinnedPrompt]);
  ...
  const stableMsgKey = useCallback((m: ChatMessage): string => {
    ...
    if (!id) {
      id = `mid-${msgIdSeq.current++}`;
      msgIds.current.set(m, id);
    }
    return id;
  }, []);
  ...
  const virtualKey = useCallback(
    (it: DisplayItem, i: number) =>
      rowKeys[i] ?? virtualKeyFor(it, i, stableMsgKey),
  ...
  const virt = useVirtualChat<DisplayItem>({
    ...
    streamingIndex:
      isStreaming && displayItems.length > 0
  ...
  const allQueuedMessages = useMemo(
    () => messages.filter((m) => m.role === "queued"),
    [messages],
  );
  ...
  const steer = useCallback(() => {
    ...
    if (isInterceptedSlashCommand(raw)) {
      ...
      void interceptSlashCommand(cmdTxt, originSlot, dispatch).then((res) => {
        ...
        const onScreen =
          originSlot === activeSlotRef.current &&
    ...
    // attachment metadata.
    const activePastes = pasteBlocksRef.current;
    const llmTxt = activePastes.length
      ? expandPasteTokens(txt, activePastes)
      : txt;
    ...
    displayItems.forEach((item, di) => {
      if (item.kind === "turn") {
        for (const ti of item.items) {
          ...
          else if (ti.kind === "group")
            ti.msgs.forEach((_, mi) => map.set(ti.startIdx + mi, di));
        }
      } else if (item.kind === "single") map.set(item.idx, di);
      else if (item.kind === "group")
        item.msgs.forEach((_, mi) => map.set(item.startIdx + mi, di));
    });
    return map;
  }, [displayItems]);
  ...
  const jumpToLoadedPinnedMessage = useCallback(
    (messageTs: string, mid?: string): boolean => {
      ...
      const msgIdx = mid
        ? messages.findIndex(
            (m) => (m.meta as Record<string, unknown> | undefined)?.mid === mid,
          )
  ...
  const handleJumpToPinnedMessage = useCallback(
    (
      ...
      { origin }: { origin: PendingJumpOrigin },
    ) => {
      if (jumpToLoadedPinnedMessage(messageTs, mid)) return;
      if (
        activeSlot &&
        (!cursorIsForActiveSlot || (slotHasMore && slotOldestIndex > 0))
      ) {
  ...
  useEffect(() => {
    ...
    const loadFailedNotice =
      pendingPinnedJump.origin === "earlier" ||
      pendingPinnedJump.origin === "link"
        ? i18nT("components.chatPane.earlier_messages_load_failed")
        : notFoundNotice;
    if (
      jumpToLoadedPinnedMessage(
        pendingPinnedJump.messageTs,
        pendingPinnedJump.mid,
      )
    ) {
      pinnedJumpPageLoadsRef.current = 0;
  ...
  const handleTogglePinForMessage = useCallback(
    (
      mid: string,
      messageTs: string,
      role: "user" | "assistant",
  ...
  const pinStatus =
    ...
    (chatPinsError
      ? i18nT(
          chatPinsError === "pin"
            ? "pages.chat.pins.pin_failed"
            : chatPinsError === "pin_limit"
              ? "pages.chat.pins.pin_limit_reached"
              : "pages.chat.pins.unpin_failed",
        )
      : null);
  ...
  useEffect(() => {
    ...
    const msgIdx = messages.findIndex(
      (m) => m.role === "tool" && m.meta?.tool_call_id === focusToolCallId,
    );
    if (msgIdx < 0) return;
  ...
  useEffect(() => {
    ...
    const msgIdx =
      targetMid && messages[resolved]?.meta?.mid !== targetMid ? -1 : resolved;
    if (msgIdx < 0) {
      ...
      handleJumpToPinnedMessage(targetTs, targetMid ?? undefined, {
        origin: "link",
      });
      return;
    }
  ...
  const activeSlotTitle = filteredSlots.find(
    (s) => s.key === activeSlot,
  )?.title;
  ...
  const renderMessage = useCallback(
    (i: number, m: ChatMessage) => {
      ...
      if (hasReasoningContent(m))
        return (
          <ThinkingBlock key={key} content={m.content} disclosureKey={key} />
        );
      ...
      if (m.role === "tool") {
        ...
        if (wfRunId)
          return <WorkflowRunCard key={key} runId={wfRunId} message={m} />;
        ...
        const isInTrailingGroup =
          slotStateRef2.current === "tool_running" &&
          i > lastTextIdxRef.current;
        return (
          <ToolCallLine
            key={key}
            message={m}
            running={isInTrailingGroup}
            onFileOpen={handleFileOpen}
            disclosure={toolDisclosure[key]}
            disclosureKey={key}
            onDisclosureChange={setToolDisclosureFor}
      ...
      if (m.role === "nudge") {
        ...
        return (
          <NudgeCard
            ...
            message={m}
            disclosureKey={key}
            onOpenLoop={ownLoop ? () => setAutoNudgeOpen(true) : undefined}
          />
        );
      }
      if (m.kind === "stop_event" || m.meta?.kind === "stop_event")
        return (
          <StopEventCard key={(m.meta?.id as string) ?? key} message={m} />
        );
      ...
      if (m.role === "inject") {
        ...
        if (card)
          return <RecoveryCard key={key} parsed={card} disclosureKey={key} />;
      }
      if (m.role === "error")
        return (
          <ErrorCard
            ...
            content={m.content}
            onContinue={
              continuable && interrupted && i === lastErrorIdx
                ? handleContinue
                : undefined
            }
      ...
      if (m.role === "notice")
        return <NoticeCard key={key} content={m.content} />;
      ...
      if (isWorkflowCompletionMessage(m))
        return (
      ...
      if (isSubagentCompletionMessage(m))
        return (
          <SubagentCompletionCard
            ...
            activeSession={activeSlot || undefined}
            disclosureKey={key}
            onOpenPanel={handleSubagentPanelOpen}
          />
        );
      ...
      return (
        <MessageSearchScope key={key} messageIdx={i}>
          <div
            className={`group flex flex-col min-w-0 ${isUser ? "items-end" : ""} ${m.ts && m.ts === highlightTs ? "animate-msg-highlight rounded-lg" : ""}`}
          >
            <div
              className={`flex flex-col gap-0.5 min-w-0 overflow-hidden max-w-full ${isUser ? "items-end" : ""}`}
            >
              {isUser ? (
                <UserMessage
                  content={m.content}
                  ...
                  pinned={
                    m.ts && (m.meta as Record<string, unknown> | undefined)?.mid
                      ? isPinned(
                          (m.meta as Record<string, unknown>).mid as string,
                        )
                      : false
                  }
                  onTogglePin={
                ...
                (() => {
                  ...
                  const stripped = cronLabel
                    ? m.content
                        .replace(/^\[Cron notification from ".*"\]\n/, "")
                  ...
                  const cleanContent = isNoteRow(m)
                    ? parseOptions(stripped).text
                    : stripped;
                  return (
                    <>
                      {cronLabel && (
                        <span className="text-muted text-[11px] leading-4 font-medium px-1 mb-1">
                          <Clock className="lucide-inline" /> {cronLabel}
                        </span>
                      )}
                    ...
                    refuses at its first guard and a `/chat?sid=` link gains `target="_blank"`. */}
                      <div
                        className="msg-content px-4 py-3 text-sm leading-6 whitespace-pre-wrap rounded-lg bg-warn-subtle text-text ring-1 ring-inset forced-colors:border ring-warn/30 rounded-bl-[4px] overflow-hidden min-w-0"
                        style={{
                          overflowWrap: "anywhere",
                          wordBreak: "break-word",
                        }}
                      >
                        <MessageErrorBoundary rawContent={cleanContent}>
                          <MarkdownRenderer
                    ...
                    mono was actually there for. */}
                      {chatConfig.showTimestamps && msgTime && (
                        <span
                          className="text-muted text-[12px] leading-4 tabular-nums px-1"
                          title={msgTimeFull}
                        >
                          {msgTime}
                        </span>
                ...
                <div className="flex flex-col gap-0">
                  <AssistantMessage
                    suppressSteerAck={turnHadPolicyBlock(
                      messagesRef.current,
                      i,
                    )}
                    prevUserText={prevUserTextFor(messagesRef.current, i)}
                    linkPreviews={linkPreviewsOn}
                    content={m.content}
                    isStreaming={isStreaming}
                    isRegenerating={
                      regenerating && i === lastTextIdxRef.current
                    }
                    onFileOpen={handleFileOpen}
                    onFolderOpen={handleFolderOpen}
                    ...
                    fileChanges={
                      (m.meta as Record<string, unknown> | undefined)
                        ?.file_changes as FileChangeEntry[] | undefined
                    }
                    turnStats={
                      chatConfig.showTurnStats
                        ? ((m.meta as Record<string, unknown> | undefined)
                            ?.turn_stats as TurnStats | undefined)
                        : undefined
                    }
                    ...
                    pinned={
                      m.ts &&
                      (m.meta as Record<string, unknown> | undefined)?.mid
                        ? isPinned(
                            (m.meta as Record<string, unknown>).mid as string,
                          )
                        : false
                    ...
                    showFooter={(() => {
                      ...
                      for (let j = i + 1; j < messagesRef.current.length; j++) {
                        ...
                        if (
                          ...
                          later.role === "streaming"
                        )
                          return false; // not last assistant in turn
                      }
                    ...
                    onFork={embedded && !popout ? undefined : handleFork}
                    onPlanFromHere={
                      embedded && !popout ? undefined : handlePlanFromHere
                    }
                    forkIndex={forkIndex}
                    forkMessageId={canResolveOnServer ? messageId : undefined}
                    onLoadEarlier={
                      cursorIsForActiveSlot ? handleLoadEarlier : undefined
                    }
                    loadingOlder={loadingOlder}
                    earlierRemaining={slotOldestIndex}
  ...
  const renderTurnItem = useCallback(
    (it: TurnItem, _j: number) => {
      ...
      if (it.kind === "single" && isHiddenInvisibleAssistantRow(it.msg))
        return null;
      return (
        <div
          key={turnLeadKey(it, stableMsgKey)}
          className={`px-4 mx-auto w-full py-1`}
          style={{ maxWidth: "var(--mc-content-width, 900px)" }}
        >
          {it.kind === "group"
            ? (() => {
                const unresolvedPerms = it.msgs.filter(
                ...
                return (
                  <CollapsibleToolGroup
                    count={
                      it.msgs.filter((m) => m.role !== "permission").length
                    }
                    disclosureKey={`ctg-${turnLeadKey(it, stableMsgKey)}`}
                    hasPermission={false}
                    isRunning={false}
                    permissionMeta={
                      unresolvedPerms.at(-1)?.meta as
                        Record<string, unknown> | undefined
                    }
                    pendingPermCount={unresolvedPerms.length}
                    onApprove={(() => {
                      const aid = unresolvedPerms.at(-1)?.meta?.approval_id as
                        string | undefined;
  ...
  const drawerTravel = useCallback(
    () =>
      Math.max(
        0,
        (window.innerWidth || 0) - DRAWER_UNCOVERED_PX + safeAreaLeft(),
      ),
    [],
  );
  /**
  ...
  const [sideOverlayPhase, setSideOverlayPhase] = useState<
    "closed" | "open" | "closing"
  >("closed");
  ...
  useEffect(
    () =>
      registerDrawerTargets(sideOverlayX, {
  ...
  useEffect(() => {
    ...
    if (sidePanelWantsMount) {
      ...
      if (sideOverlayPhaseRef.current === "closed")
        sideOverlayX.set(window.innerWidth || 0);
  ...
  const revealSourceLink = useCallback(
    (slot: string, chip: { url: string; kind: SourceLinkKind }): boolean => {
      const link = parseSourceLinkUrl(
        chip.url,
        sourceHostsRef.current,
        jiraSourceHostsRef.current,
      );
  ...
  useEffect(() => {
    const onPreviewExpand = (e: Event) => {
      ...
      if (expanded) {
        closeSidebar();
        if (sidebarAutoHidden.current === null)
          sidebarAutoHidden.current = sidebarPinnedRef.current;
    ...
    return () =>
      window.removeEventListener(PREVIEW_EXPAND_EVENT, onPreviewExpand);
  ...
  useEffect(() => {
    if (!sidebarOpen) setExpandFrom(null);
  }, [sidebarOpen]);
  ...
  const flyoutNew = useCallback(() => {
    const effectiveMode = loadChatConfig().defaultAutopilot
      ? "orchestrator"
      : mode || "";
    flyout.close();
    ...
    focusComposerAfter(
      dispatch(
        createSlot({ agent: defaultAgent || undefined, mode: effectiveMode }),
      ).unwrap(),
    );
  ...
  const panelReserve = isMobile
    ? undefined
    : (sidebarOpen ? effectiveSidebarWidth : 0) + CHAT_PANE_MIN;
         ...
         attribute carries a guarantee about a case it cannot affect. */
          ...
          hands that flyout's rect to the drawer so the panel grows out of it. */}
            {!isMobile &&
              ...
              filteredSlots.length > 0 && (
                <button
                  ...
                  title={
                    sidebarOpen
                      ? i18nT("pages.chatPage.hide_sessions")
                      : i18nT("pages.chatPage.show_sessions")
                  }
                  aria-label={
                    sidebarOpen
                      ? i18nT("pages.chatPage.hide_sessions_sidebar")
                      : i18nT("pages.chatPage.show_sessions_sidebar")
                  }
                >
                  {sidebarOpen ? (
                    <PanelLeftLight size={16} />
                  ) : (
                    <PanelLeftSolid size={16} />
                  )}
                </button>
              )}
            <AnimatePresence>
            ...
            {embedMode === "chat" ? null : embedMode === "sessions" ? (
              ...
              <OverlayDrawer
                open={isMobile ? drawerMounted : sidebarOpen}
                width={
                  isMobile
                    ? Math.max(0, winW - DRAWER_UNCOVERED_PX)
                    : effectiveSidebarWidth
                }
                dragging={sidebarDragging}
                slideX={isMobile ? drawerX : undefined}
                slideRef={drawerPanelRef}
                morph={!isMobile}
              ...
              >
                <ChatSidebar
                  ...
                  onOpenSlotInNewTab={
                    ownsSessionTabs ? openSlotInNewTab : undefined
                  }
          ...
          menu (sidebar row or header) via the ChatPage-scoped TagPopover context. */}
            ...
            {embedMode !== "sessions" && (
              <div
                ref={setChatPaneEl}
                className={`relative flex flex-col bg-bg min-w-0 min-h-0 h-full overflow-hidden ${(activityOpen && !activitySlot) || search.isOpen ? "flex-[1_1_60%]" : "flex-1"}`}
                style={
                  {
                    ...
                    ...(!sidebarOpen && !isMobile
                      ...
                      : {}),
                    "--mc-content-width":
                      CONTENT_WIDTH[chatConfig.contentWidth].messages,
                    "--mc-input-width":
                      CONTENT_WIDTH[chatConfig.contentWidth].input,
              ...
              >
                {snipFrame && (
                  <SnipOverlay
                    frame={snipFrame}
                    onComplete={(f) => {
                      uploadFiles([f], snipSlotRef.current);
                      setSnipFrame(null);
                    }}
                ...
                {uploadError && (
                  ...
                  >
                    ...
                    <button
                      ...
                      aria-label={i18nT("pages.chatPage.dismiss_upload_error")}
                ...
                {sidError && (
                  ...
                  >
                    ...
                    <button
                      onClick={() => setSidError("")}
                      aria-label={i18nT("pages.chatPage.dismiss_error")}
                      className="text-muted hover:text-text text-lg leading-none"
                    >
                      &times;
                    </button>
            ...
            arriving from the other three paths. */}
                {unresumableResume && (
                  <div
                    ...
                    data-testid="unresumable-resume-error"
                  >
                    <ErrorNotice
                ...
                {isMobile &&
                  ...
                  !(activeSlot && (messages.length > 0 || slotRunning)) && (
                    <div className="fixed top-safe-offset-[42px] left-safe ml-2 z-10">
                      <button
                        className="p-2 rounded-lg text-muted hover:text-text bg-bg-elevated border border-border shadow-sm cursor-pointer"
                        onClick={openSidebar}
            ...
            strip and the set can never disagree about whose surface this is. */}
                ...
                {splitMode && splitFeatureEnabled ? (
                  ...
                  <div className="flex-1 flex flex-col items-center justify-center gap-4 px-8">
                    <EmptyState
                      ...
                      title={i18nT("pages.chatPage.what_can_i_do_for_you")}
                      subtitle={i18nT(
                        "pages.chatPage.start_a_new_chat_to_begin",
                      )}
                    />
                    <Btn
                      primary
                ...
                blurs the input, which commits and closes the editor. */}
                      <div
                        className={`absolute top-0 left-0 right-1.5 ${editingTitle ? "z-[47]" : "z-[45]"} pointer-events-none`}
                        style={
                          { WebkitAppRegion: "no-drag" } as React.CSSProperties
                        }
                      >
                  ...
                  start of the slide. */}
                    ...
                    view of container x 44 (button 8+28 + 8px gap). */}
                          ...
                          {embedMode !== "chat" && isMobile && (
                            <button
                              className="p-1 rounded-md text-muted hover:text-text cursor-pointer bg-transparent border-none pointer-events-auto"
                              onClick={() =>
                                mobileSessions ? closeSidebar() : openSidebar()
                              }
                              aria-label={i18nT(
                                "pages.chatPage.toggle_sessions",
                              )}
                            >
                          ...
                          <div className="group/header flex min-w-0 items-stretch gap-0.5 pointer-events-auto">
                            <div className="flex items-center rounded-l-md rounded-r-[2px] px-1.5 py-0.5 group-hover/header:bg-bg-hover transition-colors">
                              <ChatHeaderMenu
                                ...
                                onRename={
                                  activeSlot
                                    ? () => {
                                        setEditingTitleSlot(activeSlot);
                                        setTitleDraft(title);
                                      }
                                    : undefined
                                }
                            ...
                            {editingTitle ? (
                              <div className="flex min-w-0 flex-1 items-center gap-1 px-1.5 py-0.5 rounded-l-[2px] rounded-r-md bg-bg-hover">
                                ...
                                <Input
                                  className="session-header-title text-sm font-semibold text-muted font-body bg-transparent border-0 rounded-none p-0 m-0 min-w-0 flex-1 outline-none md:max-w-[50vw] focus:!shadow-none"
                                  size={Math.min(
                                    Math.max(titleDraft.length + 2, 6),
                                    80,
                                  )}
                                  autoFocus
                                  value={titleDraft}
                                  ...
                                  {...titleIme.bindComposition<HTMLInputElement>(
                                    {
                                      onBlur: () => {
                                        if (
                              ...
                              <div className="cursor-text flex min-w-0 items-center gap-1 px-1.5 py-0.5 rounded-l-[2px] rounded-r-md group-hover/header:bg-bg-hover transition-colors">
                                <Clickable
                                  ...
                                  onClick={() => {
                                    if (
                                      activeSlot &&
                                      generatingTitleSlots.has(activeSlot)
                                    )
                                      return;
                                    setEditingTitleSlot(activeSlot);
                                    setTitleDraft(title);
                                ...
                                >
                                  ...
                                  <TypewriterText
                                    text={title}
                                    className="session-header-title text-sm font-semibold text-muted font-body truncate min-w-0 md:max-w-[50vw]"
                                  />
                                  <Pen
                                    size={13}
                                    className="shrink-0 text-muted opacity-0 group-hover/header:opacity-60 transition-opacity"
                                  />
                                </Clickable>
                                {activeSlot &&
                                  (generatingTitleSlots.has(activeSlot) ? (
                                    ...
                                    <Btn
                                      aria-label={i18nT(
                                        "pages.chatPage.regenerate_title_with_llm",
                                      )}
                                      ...
                                      onClick={(e) => {
                                        ...
                                        if (
                                          ...
                                          return;
                                        const slot = activeSlot;
                                        setGeneratingTitleSlots((prev) =>
                                          new Set(prev).add(slot),
                                        );
                                        api
                                          .generateTitle(slot)
                                          .then((r) => {
                                            /* title is redacted server-side via redact_exfiltration_urls + redact_credentials */ if (
                                              r.title
                                            )
                                              dispatch(
                                                sseSlotTitle({
                  ...
                  Inside the popout window itself the same spot carries Return. */}
                            {popout ? (
                              <Clickable
                                ...
                                aria-label={i18nT(
                                  "pages.chatPage.return_to_main_window",
                                )}
                              >
                                <Undo2 size={13} />{" "}
                                {i18nT("pages.chatPage.return")}
                              </Clickable>
                            ) : (
                              !embedMode &&
                              activeSlot &&
                              (activePoppedOut ? (
                                <Clickable
                                  ...
                                  aria-label={i18nT(
                                    "pages.chatPage.focus_popped_out_window",
                                  )}
                                >
                                  <ExternalLink size={13} />{" "}
                                  {i18nT("pages.chatPage.popped_out")}
                                </Clickable>
                              ) : (
                                <Clickable
                                  ...
                                  title={i18nT(
                                    "pages.chatPage.pop_out_to_window",
                                  )}
                                  aria-label={i18nT(
                            ...
                            {!embedMode &&
                              ...
                              (splitAnchorForActive && !activeIsSplitAnchor ? (
                                <Clickable
                                  ...
                                  aria-label={i18nT(
                                    "pages.chatPage.return_to_split_view",
                                  )}
                                >
                                  <Columns2 size={13} />{" "}
                                  {i18nT("pages.chatPage.in_split")}
                                </Clickable>
                              ) : (
                                <Clickable
                                  ...
                                  aria-label={i18nT(
                                    "pages.chatPage.enter_split_view",
                                  )}
                                >
                                  <Columns2 size={14} />
                                </Clickable>
                  ...
                  line the pinned prompt sticks to (see updatePinnedPrompt). */}
                        ...
                        {pinned &&
                          (pinMinimized ? (
                            <PinnedPromptPill
                              onRestore={() => setPinMinimized(false)}
                            />
                          ) : (
                            <PinnedPrompt
                              text={pinned.text}
                          ...
                          belowRows={
                            <>
                              ...
                              {activeSlot &&
                                ...
                                !embedded &&
                                !popout &&
                                slotSwitchTarget !== activeSlot && (
                                  <div
                                    className="px-4 mx-auto w-full"
                                    style={{
                                      maxWidth:
                                        "var(--mc-content-width, 900px)",
                                    }}
                                  >
                  ...
                  reason it looked mobile-only. */}
                              <div
                                style={{ height: TRANSCRIPT_TAIL_SPACER_PX }}
                              />
                            </>
                          }
                        >
                          ...
                          {visibleDisplayItems.map((vi) => {
                            ...
                            if (
                              item.kind === "single" &&
                              isHiddenInvisibleAssistantRow(item.msg)
                            )
                              return null;
                            if (item.kind === "turn") {
                              return (
                                  ...
                                  ? (() => {
                                      ...
                                      return (
                                        <CollapsibleToolGroup
                                          ...
                                          permissionMeta={
                                            unresolvedGroupPerms.at(-1)
                                              ?.meta as
                                              | Record<string, unknown>
                                              | undefined
                                          }
                                          pendingPermCount={
                                            unresolvedGroupPerms.length
                                          }
                                          onApprove={(() => {
                                            const aid = unresolvedGroupPerms.at(
                                              -1,
                                            )?.meta?.approval_id as
                                              string | undefined;
                                            if (!aid) return approve;
                ...
                border. */}
                      <div
                        ...
                        style={{
                          height:
                            TRANSCRIPT_MASK_ABOVE_PX +
                            COMPOSER_MASK_OVERSHOOT_PX,
                  ...
                  overlay one, so the capped box does not carry a permanent bar. */}
                        <div
                          ref={composerBandRef}
                          className="max-h-[50svh] overflow-y-auto overscroll-contain scrollbar-overlay pb-[11px] mb-[-11px]"
                  ...
                  calls and goes stale when the user clicks a tab in the panel. */}
                          {!(
                            activityOpen &&
                            !search.isOpen &&
                            tabsCtl.tabs.find((t) => t.id === tabsCtl.activeId)
                              ?.kind === "subagents"
                          ) && <SubagentProgressBar slot={activeSlot} />}
                          {!(
                            activityOpen &&
                            !search.isOpen &&
                            tabsCtl.tabs.find((t) => t.id === tabsCtl.activeId)
                              ?.kind === "workflows"
                          ) && <WorkflowProgressBar slot={activeSlot} />}
                          <SubagentDeliveryProgress
                            count={systemDeliveryCount}
                          />
                          <QueueStack
                        ...
                        {flyingQuote && (
                          <FlyingQuote
                            text={flyingQuote.text}
                            from={flyingQuote.from}
                            targetRef={inputAreaRef}
                            onComplete={() => setFlyingQuote(null)}
                          />
                        )}
                  ...
                  concludes the click did nothing and presses again. */}
                          {refusedPress && (
                            ...
                            >
                              <ErrorNotice
                                title={i18nT(
                                  REFUSED_PRESS_TITLE_KEYS[refusedPress.action],
                                )}
                          ...
                          {showHistorySuggestions && (
                            <div className="absolute left-0 right-0 bottom-full mb-1 mx-auto w-full max-w-[760px] border border-border rounded-lg bg-card overflow-hidden animate-scale-in z-50 shadow-lg flex flex-col max-h-[min(300px,40vh)]">
                              <div className="px-3.5 py-2.5 border-b border-border shrink-0">
                                <span className="text-[12px] font-semibold text-muted tracking-[.02em]">
                                  {i18nT(
                                    "pages.chatPage.continue_a_previous_chat",
                                  )}
                                </span>
                              </div>
                              <div
                              ...
                              <div className="px-3.5 py-2 border-t border-border flex justify-end shrink-0">
                                <span className="text-[11px] text-muted-strong">
                                  {i18nT("pages.chatPage.esc_to_dismiss")}
                                </span>
                              </div>
                            </div>
                          )}
                 ...
                 louder button here. */
                        ...
                        <VoiceDisabledModal
                          ...
                          onOpenSettings={() => {
                            ...
                            navigate(
                              embedded ? "/embed/settings" : "/settings/voice",
                            );
                          }}
                        />
                          ...
                          createPortal(
                            ...
                            <div
                              ...
                              style={(() => {
                                ...
                                return {
                                  bottom:
                                    window.innerHeight - agentBtnRect.top + 4,
                                  left,
                                };
                              })()}
                            >
                              <div className="px-1.5 pt-1.5 pb-1">
                                <Input
                                  ...
                                  type="text"
                                  aria-label={i18nT(
                                    "pages.chatPage.filter_agents",
                                  )}
                                  placeholder={i18nT(
                                    "pages.chatPage.type_to_filter",
                                  )}
                                  value={agentFilter}
                                  onChange={(e) =>
                              ...
                              <div
                                role="listbox"
                                aria-label={i18nT("pages.chatPage.agent_list")}
                    ...
                    a rejected request indistinguishable from a successful one. */}
                              {!embedded && (
                                <DefaultAgentRow
                                  ...
                                  onSetDefault={() =>
                                    toggleDefaultAgent(activeAgentName)
                                  }
                                />
                              )}
                              {!embedded && (
                        ...
                        {modelDropdown &&
                          ...
                          createPortal(
                            <ModelEffortDropdown
                              ...
                              pinModelUnavailable={pinIsWithheld(
                                _modelPinActive,
                                shownModel,
                              )}
              ...
              <DetailPanel
                ...
                title={
                  <SearchBar
                    ...
                    goTo={search.goTo}
                    scopeLimited={searchScopeIsLimited({
                      slotHasMore,
                      cursorIsForActiveSlot,
                    })}
                  />
              ...
              >
                {search.matches.length > 0 ? (
                  ...
                  <div className="px-4 py-3 text-[13px] text-muted">
                    {search.term
                      ? i18nT("pages.chatPage.no_results")
                      : i18nT(
                          "pages.chatPage.type_to_search_this_conversation",
                        )}
                  </div>
                )}
              </DetailPanel>
              ...
              {(isMobile
                ...
                >
                  <SidePanel
                    ...
                    projectDir={currentSlot?.project || undefined}
                    navLinks={chatNav.links}
                    navResolving={chatNav.resolving}
                    sources={panelSources}
                    selectedSourceUrl={selectedSourceUrl}
                    onSelectSource={selectSourceUrl}
                    onReconcileSource={reconcileSourceUrl}
                    issues={panelIssues}
                    selectedIssueUrl={selectedIssueUrl}
                    onSelectIssue={selectIssueUrl}
                    onReconcileIssue={reconcileIssueUrl}
                    onAddSourceToChat={addSourceCommentToChat}
                    onSubmitComments={submitComments}
                    onFileSave={handleFileSave}
                    onClose={toggleAct}
                    pins={chatPins}
                    pinsLoading={chatPinsLoading}
                ...
                <AnimatePresence initial={false}>
                  {shouldMountSidePanel({
                    ...
                    <motion.div
                      ...
                      className={
                        sidePanelDock === "bottom"
                          ? "w-full overflow-visible flex flex-col justify-end"
                          : "h-full overflow-visible flex justify-end"
                      }
                      style={
                        isSidePanelHidden({
                          activityOpen,
