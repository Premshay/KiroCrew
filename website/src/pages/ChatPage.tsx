...
import {
  featureRequestRefusalIsNewest,
  sessionStartRepeatIsNewest,
} from './chat/transcriptRenderers'
import {
  useDrawerSwipe,
  animateDrawer,
...
import {
  addTab as addDockTerminal,
  removeTab as removeDockTerminal,
  hasTab as hasDockTerminal,
...
const stagedIdentity = (
  ...
  sessions: readonly SessionRef[] | undefined,
...
export default function ChatPage({
  ...
  const foregroundCreateIdRef = useRef(foregroundCreateId)
  foregroundCreateIdRef.current = foregroundCreateId
  const lastCreatedActivation = useAppSelector((s) => s.chat.lastCreatedActivation)
  const lastCreatedActivationRef = useRef(lastCreatedActivation)
  lastCreatedActivationRef.current = lastCreatedActivation
  ...
  const folderSuggestion = useAppSelector((s) =>
    s.chat.activeSlot ? s.chat.folderSuggestions?.[s.chat.activeSlot] : undefined,
  )
  ...
  const tipTemporary = useAppSelector(
    (s) =>
      s.dashboard.slots.find((sl) => sl.key === s.chat.activeSlot)?.memory_mode === 'temporary',
  )
  ...
  const {
    ...
    defaultAgent,
  } = useAgents(refreshTrigger, activeSlot ?? undefined, activeSlotProject)
  ...
  const { onListKeyDown: onAgentListKeyDown } = useListboxKeyboard({
    ...
    onEnterSingleMatch: () => {
      ...
      if (a) {
        switchAgent(a.name, a.selection_kind)
  ...
  const { onListKeyDown: onModelListKeyDown } = useListboxKeyboard({
    ...
    onEnterSingleMatch: () => {
      pickModel(filteredModels[0].name)
    },
  ...
  const showActionError = useCallback((message: string, title?: string) => {
    ...
    setActionError((prev) =>
      prev && prev.message === message && prev.title === title ? prev : { title, message },
    )
  }, [])
  ...
  const automationSnapshot = useQuery({
    ...
    queryFn: async () => {
      ...
      const hasFullLegacyRecord = legacy.loop !== null && isFullLegacyAutomationRecord(legacy.loop)
      const legacySnapshot = !hasFullLegacyRecord ? null : normalizeAutomationRecord(legacy.loop)
      const structuredRecord =
        structured.monitor === null ? null : normalizeAutomationRecord(structured.monitor)
      if (
        (hasFullLegacyRecord && !legacySnapshot) ||
        (structured.monitor !== null && structuredRecord?.kind !== 'structured_monitor')
      ) {
        throw new Error('Invalid session automation snapshot')
      }
      const legacyRecord = legacySnapshot?.kind === 'legacy_goal_loop' ? legacySnapshot : null
  ...
  const automationCreationReady =
    !!automation || (automationSnapshot.isSuccess && !automationSnapshot.isFetching)
  ...
  useLayoutEffect(() => {
    ...
    if (!el) return
    const compute = () =>
      setScrollerWidthBucket(Math.min(Math.round(el.clientWidth / 16) * 16, 944))
    compute()
  ...
  const mountIndexRef = useRef<(index: number, opts?: { unionOnly?: boolean }) => boolean>(
    () => false,
  )
  ...
  const raisePrefillHint = useCallback(() => {
    setPrefillHint(true)
  ...
  const processErrorHandoffs = useCallback(async () => {
    ...
    const ownsLifecycle = () =>
      errorHandoffMountedRef.current && errorHandoffLifecycleRef.current === lifecycle
  ...
  useEffect(() => {
    ...
    const drain = () => {
      ...
      while ((prompt = consumeChatHandoff()) !== null) {
        ...
        if (prompt !== errorHandoffActiveRef.current && !handoffQueue.includes(prompt))
  ...
  useEffect(() => {
    if (foregroundCreateId && !noCarryCreateIdsRef.current.has(foregroundCreateId)) {
      createCarryRef.current.set(foregroundCreateId, {
        origin: composerSlotRef.current,
        baseline: inputRef.current,
        staged: stagedIdentity(stagedNowRef.current.files, stagedNowRef.current.sessions),
      })
    }
  }, [foregroundCreateId])
  ...
  useEffect(() => {
    ...
    if (prevSlot.current)
      setSessionRefDraft(sessionRefDrafts.current, prevSlot.current, stagedNow.sessions)
    ...
    const carry =
      activation && activation.slot === activeSlot
        ? createCarryRef.current.get(activation.requestId)
        : undefined
    ...
    while (createCarryRef.current.size > 8)
      createCarryRef.current.delete(createCarryRef.current.keys().next().value as string)
    ...
    const nothingStaged =
      !!carry &&
      carry.staged === NOTHING_STAGED &&
      stagedIdentity(stagedNow.files, stagedNow.sessions) === NOTHING_STAGED
    if (carry && activeSlot && prevSlotVal === carry.origin && nothingStaged) {
      ...
      if (typed !== null) {
        ...
        const moved = carryPastes(
          typed,
          pruneBlocksUtil(typed, blocks),
          pasteDrafts.current[activeSlot] ?? [],
        )
        carried = moved.text
    ...
    if (!activeSlot && pendingCreate && !noCarryCreateIdsRef.current.has(pendingCreate)) {
      createCarryRef.current.set(pendingCreate, {
        ...
        staged: stagedIdentity(
          activeSlot ? fileDrafts.current[activeSlot] : undefined,
          activeSlot ? sessionRefDrafts.current[activeSlot] : undefined,
        ),
      })
    }
    ...
    setPasteBlocks(activeSlot ? (pasteDrafts.current[activeSlot] ?? []).map((b) => ({ ...b })) : [])
    ...
    setPendingSessions(
      activeSlot ? (sessionRefDrafts.current[activeSlot] ?? []).map((r) => ({ ...r })) : [],
    )
  ...
  useEffect(
    () => () => {
      ...
      if (prevSlot.current)
        setSessionRefDraft(sessionRefDrafts.current, prevSlot.current, pendingSessionsRef.current)
      flushDrafts()
    },
    [flushDrafts],
  )
  ...
  const { rect: projectBtnRect, anchorTo: anchorProjectBtn } =
    useAnchoredTriggerRect(projectPickerOpen)
  ...
  useEffect(() => {
    ...
    if (!appSlotLaunch) {
      ...
      if (activeSlot !== intent.slotKey) {
        if (intent.message)
          setActionError({
            message: i18nT('appChatLaunch.unsent', {
              error: i18nT('appChatLaunch.cancelled'),
  ...
  const clearFollowUpOwnership = useCallback(() => {
  ...
  const send = useCallback(
    async (
      ...
      const widgetOrigin =
        !isolated && !!widgetPrefillRef.current && raw.includes(widgetPrefillRef.current)
      if (!isolated) widgetPrefillRef.current = null
      if (
        !raw &&
        (isolated || (!pendingFilesRef.current.length && !pendingSessionsRef.current.length))
      )
        return false
      ...
      const folderCardAtSend =
        !isolated && entrySendSlot && entrySendSlot === uiSlot
        ...
        try {
          created = await dispatch(
            createSlot({
              ...
              agent_kind: pendingAgentRef.current ? pendingAgentKindRef.current : undefined,
          ...
          if (isolated) {
            ...
            const failure = i18nT('pages.chatPage.send_failed_with_error', {
              error: createFailReason(e),
            })
            setActionError({
              message: i18nT('appChatLaunch.unsent', { error: failure, message: raw }),
      ...
      const failLocalTurn = (message: ChatMessage) => {
        if (isolated) {
          ...
          setActionError({
            message: i18nT('appChatLaunch.unsent', { error: message.content, message: raw }),
            preserveOnSwitch: true,
          })
          return
        }
  ...
  const switchAgent = useCallback(
    async (agentName: string, kind?: 'member' | 'template') => {
      if (!activeSlot) {
  ...
  const revealAppInPanel = useCallback(
    (toolCallId: string) => {
      ...
      const payload = activeSlot
        ? boundStore.getState().chat.mcpApps?.[mcpAppKey(activeSlot, toolCallId)]
        : undefined
      tabsCtlRef.current?.openApp(
        toolCallId,
        mcpAppTabTitle(payload, i18nT('pages.chatPage.mcp_app_tab_title')),
  ...
  const handleAddToContext = useCallback((absPath: string, kind: 'file' | 'dir') => {
    ...
    if (kind === 'dir') {
      ...
      const dup =
        ...
        parseDirTokens(inputRef.current).some((t) => t.rel.replace(/\\/g, '/') === relSlash)
      if (!dup) {
  ...
  const followupStartInWorktree = useCallback(
    async (item: FollowupItem) => {
      ...
      const path = res?.path
      if (!path)
        throw new Error(res?.error || i18nT('pages.chatPage.worktree_creation_returned_no_path'))
      let slotKey = ''
      try {
  ...
  const _slotAgentName =
    currentSlot && !currentSlot.model ? currentSlot.agent || defaultAgent || 'default' : ''
  ...
  const shownModel = displayModel(
    ...
    codexPairModels
      ? modelWithoutEffort(currentSlot?.served_model || '')
  ...
  // MERGE-REVIEW: the live session's selection capabilities win when present;
  // before a session answers, the crew's advertised levels (fork) replace
  // upstream's bare model-name allowlist.
  const effortSupported =
    ...
    (selectionCapabilities
      ...
      : effortSupportedForCrew(crewEffortLevels, shownEffortModel))
  ...
  const remoteContextWindow = useMemo(() => {
    ...
    return (
      remoteCrew.capabilities?.models.find(
        (m) => (codexPairModels ? modelWithoutEffort(m.model_name) : m.model_name) === picked,
      )?.context_window || 0
    )
  ...
  const _modelPinPinned =
    !!_modelPinCfg?.model &&
    (codexPairModels ? modelWithoutEffort(_modelPinCfg.model) : _modelPinCfg.model) ===
      _modelPinActive &&
  ...
  const effectiveEffort =
    ...
    legacyCodexEffort(currentSlot?.model || '', '', codexPairModels) ||
    defaultEffort
  ...
  const splitAnchorForActive = useMemo(() => {
    ...
    return sessionSlots(loadLayout(anchor)).filter((k) => liveKeys.has(k)).length >= 2
      ? anchor
      : null
  ...
  const renderUserContentCb = useCallback(
    ...
    (c: string, mt: Record<string, unknown> | undefined, ts?: string) =>
      renderUserContent({
        content: c,
        meta: mt,
  ...
  const streamTick =
    lastRole === 'streaming' ? (messages[messages.length - 1]?.content.length ?? 0) : 0
  ...
  const earlierBarInView = useCallback(() => {
    ...
    return earlierAffordanceInView(br ? { top: br.top, bottom: br.bottom } : null, {
      top: vr.top,
      bottom: vr.bottom,
    })
  }, [])
  ...
  const virt = useVirtualChat<DisplayItem>({
    ...
    streamingIndex:
      isStreaming && renderedDisplayItems.length > 0 ? renderedDisplayItems.length - 1 : undefined,
    ...
    (opts?: { auto?: boolean }) => {
      ...
      if (isInterceptedSlashCommand(raw)) {
        ...
        void interceptSlashCommand(cmdTxt, originSlot, dispatch).then((res) => {
          ...
          const onScreen =
            originSlot === activeSlotRef.current && composerSlotRef.current === originSlot
          if (onScreen) {
  ...
  const navigateToTurn = useCallback(
    (displayIndex: number, opts?: { instant?: boolean }) => {
  ...
  const handleJumpToPinnedMessage = useCallback(
    (messageTs: string, mid: string | undefined, { origin }: { origin: PendingJumpOrigin }) => {
  ...
  useEffect(() => {
    ...
    const loadFailedNotice =
      pendingPinnedJump.origin === 'earlier' || pendingPinnedJump.origin === 'link'
  ...
  const handleTogglePinForMessage = useCallback(
    (mid: string, messageTs: string, role: 'user' | 'assistant', content: string) => {
      if (isPinned(mid)) {
  ...
  const pinError =
    ...
    (chatPinsError && chatPinsError !== 'pin_limit'
      ? i18nT(
          chatPinsError === 'pin' ? 'pages.chat.pins.pin_failed' : 'pages.chat.pins.unpin_failed',
        )
      : null)
  const pinStatus =
    pinNotice ?? (chatPinsError === 'pin_limit' ? i18nT('pages.chat.pins.pin_limit_reached') : null)
  ...
  const { renderers: chatPageRenderers, fallback: bubbleRenderer } = useMemo<{
              ...
              >
                {isUser ? (
                  <UserMessage
                    ...
                    canEdit={
                      !slotRunning && !regenerating && !!activeSlot && !activeSlotRemoteBound
                    }
                    ...
                    pinned={
                      m.ts && (m.meta as Record<string, unknown> | undefined)?.mid
                        ? isPinned((m.meta as Record<string, unknown>).mid as string)
                        : false
                    }
                    onTogglePin={
                      m.ts && (m.meta as Record<string, unknown> | undefined)?.mid
                  ...
                  (() => {
                    ...
                    return (
                      <>
                        ...
                        {!cronLabel && appLabel && (
                          <span
                            className="text-muted text-[11px] leading-4 font-medium px-1 mb-1 cursor-help"
                            title={i18nT('components.mcpApp.from_app_tooltip')}
                          >
                            <AppWindow className="lucide-inline" />{' '}
                            {i18nT('components.mcpApp.from_app', { app: appLabel.split('/')[0] })}
                          </span>
                        )}
                    ...
                    refuses at its first guard and a `/chat?sid=` link gains `target="_blank"`. */}
                        <div
                          className="mc-message-font-scope msg-content px-4 py-3 leading-relaxed rounded-lg bg-warn-subtle text-text ring-1 ring-inset forced-colors:border ring-warn/30 rounded-bl-[4px] overflow-hidden min-w-0"
                          style={{
                            overflowWrap: 'anywhere',
                            wordBreak: 'break-word',
                            fontSize: 'var(--mc-message-font-size, 14px)',
                          }}
                        >
                  ...
                  <div className="flex flex-col gap-0">
                    <AssistantMessage
                      revealActions={!!activeSlot && voiceRecoverySlot === activeSlot}
                      suppressSteerAck={turnHadPolicyBlock(messagesRef.current, i)}
                      prevUserText={prevUserTextFor(messagesRef.current, i)}
                      shareEnabled={socialShareOn}
                      linkPreviews={linkPreviewsOn}
                      content={m.content}
                      isStreaming={isStreaming}
                      isRegenerating={regenerating && i === lastTextIdxRef.current}
                      onFileOpen={handleFileOpen}
                      onFolderOpen={handleFolderOpen}
                      ...
                      mode={mode}
                      fileChanges={
                        (m.meta as Record<string, unknown> | undefined)?.file_changes as
                      ...
                      showFooter={(() => {
                        ...
                        const stats = (m.meta as Record<string, unknown> | undefined)
                      ...
                      onSwitchVariant={
                        ...
                        activeSlot
                          ? (idx: number) => {
                              api.switchVariant(activeSlot, idx).catch((e: unknown) => {
                      ...
                      onLoadEarlier={cursorIsForActiveSlot ? handleLoadEarlier : undefined}
    ...
    const shared = createTranscriptRenderers({
      ...
      toolRunning: (_m, ctx) =>
        slotStateRef2.current === 'tool_running' && ctx.index > lastTextIdxRef.current,
      transcriptHot,
  ...
  const renderTurnItem = useCallback(
    (it: TurnItem, _j: number) => {
      ...
      if (it.kind === 'single' && it.msg.role === 'tool' && !it.msg.content.startsWith('🔧'))
        return null
            ...
            ? (() => {
                ...
                return (
                  <CollapsibleToolGroup
                    ...
                    isRunning={false}
                    permissionMeta={
                      unresolvedPerms.at(-1)?.meta as Record<string, unknown> | undefined
                    }
  ...
  const flyoutNew = useCallback(() => {
    ...
    focusComposerAfter(
      dispatch(createSlot({ agent: defaultAgent || undefined, mode: effectiveMode })).unwrap(),
    )
    ...
    <RowDisclosureProvider resetKey={activeSlot}>
      <TagPopoverProvider>
        ...
        <JiraHostsCtx.Provider value={jiraSourceHosts}>
         ...
         panel has its own. */
            style={
              { '--mc-message-font-size': `${chatConfig.messageFontSize}px` } as React.CSSProperties
            }
          >
            <AnimatePresence>
          ...
          hands that flyout's rect to the drawer so the panel grows out of it. */}
            ...
            {embedMode === 'chat' ? null : embedMode === 'sessions' ? (
              ...
              <OverlayDrawer
                ...
                width={isMobile ? Math.max(0, winW - DRAWER_UNCOVERED_PX) : effectiveSidebarWidth}
          ...
          menu (sidebar row or header) via the ChatPage-scoped TagPopover context. */}
            ...
            the NAME; the sentence resolves here so a locale switch re-renders it. */}
                <ErrorNotice
                  message={
                    switchSlotGone
                      ? switchSlotNoticeCopy(switchSlotGone.kind, switchSlotGone.name)
                      : ''
                  }
                ...
                {splitMode && splitFeatureEnabled ? (
                  ...
                  <div className="flex-1 flex flex-col items-center justify-center gap-4 px-8">
                    <EmptyState
                      ...
                      subtitle={i18nT('pages.chatPage.start_a_new_chat_to_begin')}
                    />
                    <Btn
                      primary
                      ...
                      onClick={() => {
                        ...
                        dispatch(
                          createSlot({
                            ...
                            agent_kind: pendingAgent ? pendingAgentKindRef.current : undefined,
                  ...
                  key gates the bar to match the paging thunk's own precondition. */}
                                {slotHasMore && cursorIsForActiveSlot && (
                                  <EarlierMessagesBar
                                    ...
                                    onFocusRelease={() => scrollerRef.current?.focus()}
                                  />
                                )}
                              </>
                            }
                                    ...
                                    ? (() => {
                                        ...
                                        return (
                                          <CollapsibleToolGroup
                                            ...
                                            isRunning={
                                              slotRunning &&
                                              displayIdx === renderedDisplayItems.length - 1
                                            }
                                            permissionMeta={
                ...
                border. */}
                  ...
                  calls and goes stale when the user clicks a tab in the panel. */}
                          {!(
                            activityOpen &&
                            !search.isOpen &&
                            tabsCtl.tabs.find((t) => t.id === tabsCtl.activeId)?.kind ===
                              'subagents'
                          ...
                          <QueueStack
                            ...
                            pendingIds={queuePendingIds}
                            fuseBelow={
                              followUpOptions.length === 0 && !knowledgeFetch.pendingKnowledge
                            }
                          />
                        </div>
                        {flyingQuote && (
                  ...
                  concludes the click did nothing and presses again. */}
                            ...
                            <div className="absolute left-0 right-0 bottom-full mb-1 mx-auto w-full max-w-[760px] border border-border rounded-lg bg-card overflow-hidden animate-scale-in z-50 shadow-lg flex flex-col max-h-[min(300px,40vh)]">
                              <div className="px-3.5 py-2.5 border-b border-border shrink-0">
                                <span className="text-[12px] font-semibold text-muted tracking-[.02em]">
                                  {i18nT('pages.chatPage.continue_a_previous_chat')}
                                </span>
                              </div>
                              <div
                                className="overflow-y-auto flex-1 min-h-0"
                                role="listbox"
                                aria-label={i18nT('pages.chatPage.previous_chats')}
                              >
                                {historySuggestions.map((s) => (
                                  <div
                                    ...
                                    onMouseDown={(e) => {
                                      e.preventDefault()
                                      handleResumeSession(s.key, s.title || s.key)
                                    }}
                                    onKeyDown={(e) => {
                                      if (e.key === 'Enter')
                                        handleResumeSession(s.key, s.title || s.key)
                              ...
                              <PendingQuestionCard
                                ...
                                onDirectSend={(text) => {
                                  ...
                                  if (
                                    slot &&
                                    isNativeCard &&
                                    selectComposerBusy(store.getState(), slot)
                                  ) {
                                    const steerSendId = mintSendId()
                                    drainPendingChunks()
                                    dispatch(
                          ...
                          {showComposerMemoryChip && (
                            ...
                            <div
                              className="relative z-10 flex justify-center px-4 pt-2 pb-2 bg-bg"
                              data-testid="composer-memory-chip"
                            >
                              <MemoryModeChip
                                memoryMode={currentSlot?.memory_mode ?? 'persistent'}
                                onSwitchMode={switchMemoryMode}
                 ...
                 withholds Resume, so the composer falls back to Send. */
                              continuable={
                                continuable &&
                                interrupted &&
                                !featureRequestRefused &&
                                !sessionStartRepeated
                              }
                              ...
                              onFollowUpSelect={(
                                ...
                                if (followUpPickedRef.current.has(o)) {
                                  ...
                                  const r = removeFollowUpOption(
                                    inputRef.current,
                                    followUpInsertedRef.current,
                                    o,
                                  )
                              ...
                              knowledgeChip={
                                knowledgeFetch.pendingKnowledge ? (
                                  <div className="flex items-start gap-1">
                                    <KnowledgeBubbleChip
                                      knowledge={{
                                        items: knowledgeFetch.pendingKnowledge.items.length,
                                        tokens: knowledgeFetch.pendingKnowledge.totalTokens,
                                        titles: knowledgeFetch.pendingKnowledge.items.map(
                                          (i) => i.title,
                                        ),
                                        content: knowledgeFetch.pendingKnowledge.items.map((i) => ({
                                          title: i.title,
                                          text: i.content.slice(0, 2000),
                                        })),
                                      }}
                                    ...
                                    <button
                                      type="button"
                                      onClick={() => knowledgeFetch.clearPending()}
                                      ...
                                      aria-label={i18nT('pages.chatPage.remove_knowledge_context')}
                                      title={i18nT('pages.chatPage.remove_knowledge_context')}
                                    >
                                      &times;
                                    </button>
                                  </div>
                        ...
                        {agentDropdown &&
                          ...
                          createPortal(
                            ...
                            <div
                              ...
                              role="dialog"
                              aria-label={i18nT('pages.chatPage.agent_selector')}
                              tabIndex={-1}
                              ...
                              style={(() => {
                                const left = Math.max(
                                  8,
                                  Math.min(agentBtnRect.left, window.innerWidth - 348),
                                )
                            ...
                            >
                              <div className="px-1.5 pt-1.5 pb-1">
                                <Input
                                  ref={agentInputRef}
                                  type="text"
                                  aria-label={i18nT('pages.chatPage.filter_agents')}
                              ...
                              >
                                <AgentDropdownList
                                  ...
                                  activeKind={currentSlot?.agent_kind}
                                  defaultAgent={defaultAgent}
                                  onSelect={(name, kind) => {
                ...
                {search.matches.length > 0 ? (
                  ...
                  <div className="px-4 py-3 text-[13px] text-muted">
                    {search.term
                      ...
                      : i18nT('pages.chatPage.type_to_search_this_conversation')}
                  </div>
                )}
              </DetailPanel>
            )}
