...
const PROVIDER_OPTIONS = ['system', 'pocket', 'piper', 'polly']
/**
 ...
 *    (the default, nothing to install), Pocket TTS or Piper (both local), or
 *    Amazon Polly (cloud). The field set switches with the provider.
        ...
        <SettingsCard>
          {voiceQ.isError ? (
            ...
            <>
              ...
              <SettingsSelect settingId="voice.provider-2" label={i18nT('pages.settings.voicePanel.provider')} description={voiceCfg.provider === 'pocket' ? i18nT('pages.settings.voicePanel.pocket_tts_runs_locally_and_streams_ogg_opus_for_on_demand_replay') : i18nT('pages.settings.voicePanel.the_built_in_engine_needs_no_setup_piper_is_offlin')} value={voiceCfg.provider} options={PROVIDER_OPTIONS} optionLabels={PROVIDER_OPTIONS.map(p => i18nT(PROVIDER_LABEL_KEY[p]))} onChange={v => setVoice({ provider: v, ...(v === 'pocket' ? { voice: 'michael' } : {}) })} disabled={voiceDisabled} />
              {isSystem ? (
                <>
                  {systemUnavailable && (
