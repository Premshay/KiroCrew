...
export default function PerformanceTab({ planeStateRef }: { planeStateRef: MutableRefObject<PlaneState> }) {
  ...
  // Persist the resource choice AND the collected samples so both survive a
  // plane flip. Writing a ref costs no re-render, so running this on every new
  // sample is cheap.
  useEffect(() => {
    planeStateRef.current = {
      ...planeStateRef.current,
      performance: { selected, history },
    }
...
function LargeGraph({ history, resource }: { history: HistoryPoint[]; resource: Resource }) {
  ...
  // is what lets a viewer tell 3 KB/s from 3 MB/s at the same bar height.
  const caption =
    resource === 'network'
  ...
  return (
    <div className="absolute inset-0 flex flex-col">
      <div className="flex-1 flex items-end gap-px px-1 pt-1">
        {bars.map((pct, i) => (
          <div
            key={i}
