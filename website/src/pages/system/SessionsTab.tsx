                  ...
                  {row.getVisibleCells().map(cell => {
                    ...
                    return (
                      <TableCell
                        ...
                        className={
                          isName
                            ? `px-3 py-1 text-left text-[12.5px] max-w-[330px] truncate ${
                          ...
                          <>
                            ...
                            {href ? (
                              <Btn
                                ...
                                // `Btn` is inline-flex, which does not shrink below its
                                // content width, so without `min-w-0` the button
                                // overflows the cell instead of ellipsizing. NOTE: this
                                // is not the whole story — a session row's name can
                                // still be clipped out of view entirely, because the
                                // expander's negative margin places this button left of
                                // the cell's content box while the cell clips overflow.
                                // Fixing that needs a real column-width model
                                // (table-fixed + colgroup), not another width hint.
