...
export type SwitchSlotArg =
  | string
  | { key: string; keepTargetOnMissing?: boolean };
...
>("chat/switchSlot", async (arg, { dispatch, getState, rejectWithValue }) => {
  ...
  try {
    ...
    if (typeof status === "number")
      return rejectWithValue({ status, message: errMessage(e) });
    throw e;
  }
});
...
>("chat/createSlot", async (opts, { getState, fulfillWithValue }) => {
  ...
  const memory_mode =
    requestedMemoryMode || (await configuredDefaultMemoryMode());
...
const chatSlice = createSlice({
  ...
  reducers: {
    ...
    sseChatMessage(
      ...
      const { slot, role, content, ts, seq, cls, meta, kind, batched } =
        action.payload;
      if (slot !== state.activeSlot) {
