...
const chatSlice = createSlice({
  ...
  reducers: {
    ...
    sseSubagentSpawn(state, action: PayloadAction<{ slot: string; id: string; task: string; agent: string; model?: string; requested_model?: string; child_session?: string; controllable?: boolean }>) {
      if (isUnsafeKey(action.payload.slot) || isUnsafeKey(action.payload.id)) return
    ...
    sseSubagentDone(state, action: PayloadAction<{ slot: string; id: string; elapsed: number; error?: string; stopped?: boolean; reported?: boolean; controllable?: boolean; outcome?: 'completed' | 'failed' | 'stopped' | 'reported'; task?: string; agent?: string; model?: string; requested_model?: string; child_session?: string; result?: string }>) {
      if (isUnsafeKey(action.payload.slot) || isUnsafeKey(action.payload.id)) return
      const subs = action.payload.slot !== state.activeSlot
        ? (state.slotActivity[safeKey(action.payload.slot)] ??= { toolLog: [], subagents: {} }).subagents
        : state.subagents
    ...
    sseSubagentSnapshot(state, action: PayloadAction<{ id: string; slot: string; task: string; agent: string; model?: string; requested_model?: string; child_session?: string; streaming: string; last_tool: string; started: number; tool_count?: number; stalled?: boolean; idle_secs?: number; controllable?: boolean }>) {
