...
import { ComposerVoiceSliceOverride, type ComposerVoiceInputProps } from '../chat-core/composer/Composer'
/**
 ...
 * mobile fixes.
 *
 * The hands-free state reaches ChatInput through the Composer root's voice
 * slice (chat-core P3-b), like every other dictation value, so these specs
 * drive it through the slice's test seam rather than props.
 */
...
const withVoice = (inputProps: Partial<ComposerVoiceInputProps>) => (
...
describe('ChatInput — hands-free toggle', () => {
  it('renders next to the mic when available, and fires its handler', () => {
    ...
    renderWithProviders(
      withVoice({ handsFreeAvailable: true, onHandsFreeToggle, onVoiceToggle: vi.fn() }),
    )
  ...
  it('is absent when hands-free is unavailable (streaming STT, config off)', () => {
    renderWithProviders(
      withVoice({ handsFreeAvailable: false, onHandsFreeToggle: vi.fn(), onVoiceToggle: vi.fn() }),
    )
  ...
  it('reflects the persisted preference via aria-pressed', () => {
    renderWithProviders(
      withVoice({ handsFreeAvailable: true, handsFreeOn: true, onHandsFreeToggle: vi.fn(), onVoiceToggle: vi.fn() }),
    )
  ...
  it('grows to the 40px touch floor where the pointer cannot hover', () => {
    renderWithProviders(
      withVoice({ handsFreeAvailable: true, onHandsFreeToggle: vi.fn(), onVoiceToggle: vi.fn() }),
    )
...
describe('ChatInput — hands-free status strip', () => {
  ...
  it('cycles listening → processing → sent with the stop hint always present', () => {
    const voice = { handsFreeAvailable: true, onHandsFreeToggle: vi.fn(), onVoiceToggle: vi.fn() }
    const { rerender } = renderWithProviders(
      withVoice({ ...voice, handsFreePhase: 'listening', voiceRecording: true }),
    )
    ...
    rerender(withVoice({ ...voice, handsFreePhase: 'processing', voiceTranscribing: true }))
    expect(screen.getByTestId('handsfree-status')).toHaveTextContent('Transcribing')

    rerender(withVoice({ ...voice, handsFreePhase: 'sent' }))
  ...
  it('still disables the mic during a plain (non-hands-free) transcription', () => {
    renderWithProviders(
      withVoice({ onVoiceToggle: vi.fn(), voiceTranscribing: true }),
    )
