...
describe('subagentCompletion parsing/detection', () => {
  ...
  it('parses the restart-recovery and delivery-timeout headers, where the glyph sits MID-line', () => {
    ...
    // after the glyph. Anchoring the glyph to end-of-line missed all three, and
    // the assistant-role timeout variant then rendered as a raw prompt bubble.
    const shapes: [string, string, string][] = [
      ['Agent `53e3e5eb` ⚠️ orphaned by gateway restart', 'interrupted', 'orphaned by gateway restart'],
      ['Agent `53e3e5eb` ❌ lost to gateway restart', 'failed', 'lost to gateway restart'],
      ['Agent `53e3e5eb` ❌ delivery timed out', 'failed', 'delivery timed out'],
    ]
    for (const [header, outcome, note] of shapes) {
      const content = SINGLE.replace('Agent `53e3e5eb` (kirocrew) completed ✅', header)
      ...
      // The words beside the glyph are the ONLY explanation these carry, so they
      // must survive into the payload rather than being dropped with the header.
