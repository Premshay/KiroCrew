...
/**
 * ChatPage.tsx is formatted with double-quoted string literals; the rest of the
 * tree is single-quoted. Every scrape below therefore accepts either quote --
 * the quote character is not part of any contract this file states, and a
 * single-quote-only pattern reads the page as EMPTY, which turns each assertion
 * vacuous instead of false.
 */
const QUOTE = `["']`
...
describe('chat role parity (ChatPage consumes the app-sdk registry)', () => {
  it('ChatPage dispatches rows through the registry, not an if-chain of its own', () => {
    expect(src).toMatch(new RegExp(`import \\{[^}]*\\bmergeRenderers\\b[^}]*\\} from ${QUOTE}\\.\\./app-sdk/messageRenderers${QUOTE}`))
    expect(src).toMatch(new RegExp(`import \\{[^}]*\\bresolveRenderer\\b[^}]*\\} from ${QUOTE}\\.\\./app-sdk/messageRenderers${QUOTE}`))
  ...
  it('the SDK defaults that render through ctx.wrapper are unreachable on this page', () => {
    ...
    const bubbleRoles = rendererBlock().match(new RegExp(`id: ${QUOTE}bubble${QUOTE},\\s*\\n\\s*roles: \\[([^\\]]*)\\]`))
    expect(bubbleRoles).not.toBeNull()
    for (const r of wrapperUsers) for (const role of r.roles) expect(bubbleRoles![1]).toMatch(new RegExp(`${QUOTE}${role}${QUOTE}`))
  })
  ...
  it('a role nobody claims falls back to the BUBBLE by reference, never to an SDK default by position', () => {
    ...
    const undrawn = rendererBlock().match(new RegExp(`id: ${QUOTE}undrawn${QUOTE},\\s*\\n\\s*roles: \\[([^\\]]*)\\]`))
    expect(undrawn).not.toBeNull()
    expect(undrawn![1]).not.toMatch(new RegExp(`${QUOTE}(system|done)${QUOTE}`))
  })
