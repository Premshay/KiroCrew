 ...
 * pixels away.
 *
 * A STEER is the exception in the other direction, and the one rule this scan
 * adds on top of `isTurnOpener`. It carries role `user`, so the opener test
 * alone admits it, but `meta.steer` (set by the `steer_push` echo) marks it as
 * injected INTO a turn already running: its row lays out between the opener and
 * that turn's reply, so admitting it hands the pin to the interruption for the
 * rest of the turn.
 */
...
function isPrompt(item: DisplayItem | undefined): boolean {
  ...
  return isTurnOpener(item.msg) && !isSteer(item.msg)
}
 ...
 * rows — `isPrompt` derives from `isTurnOpener`), not only consecutive
